...
///
/// All non-ASCII characters are written as \u / \U escapes so
/// the source itself is plain ASCII (avoids the F# 9 BiDi /
/// Trojan-Source compiler warning under TreatWarningsAsErrors).
...
let ``strips C1 control characters`` () =
    ...
    // sequences SR-1 already caps in the parser. Source uses
    // \u escape forms so the file stays plain ASCII.
    let input = "x\u0080y\u008Bz\u009Bw\u009Fq"
