...
| Cycle 49 PR-G | `335dbb9` | **Removed the deferred prefix-trim cleanup.** The "defence-in-depth" `tupleFinaliseAnnounce` + `lastAnnouncedText` prefix-trim path that Cycle 48 PR-E kept in source was identified in Cycle 49 PR-G as the cause of maintainer-reported "speech unreliable / unpredictable" symptoms (silenced duplicate commands). Cycle 49 PR-F had already replaced the prefix-trim's last legitimate use (sub-prompt cleanup) with a line-count slice. PR-G deletes the prefix-trim branch. The state-machine-driven sub-prompt announce + line-count post-Enter slicing are now the only paths. |
