...
| 4b    | Process-cleanup diagnostic             | `Ctrl+Shift+D` launches bundled `test-process-cleanup.ps1`; recurring acceptance check confirms no orphans after Alt+F4 / X-button close | **shipped** (PR #81; recurring acceptance check per SESSION-HANDOFF item 2; spec §4b. Known limitation: spawned-PowerShell NVDA reading is unreliable; in-pty-speak rework deferred per SESSION-HANDOFF item 6, now actionable since Stage 6 shipped.) |
| 4a    | Claude Code rendering substrate        | DECTCEM, DECSC/DECRC, 256/truecolor SGR, alt-screen 1049 back-buffer, OSC 52 silent drop; xUnit ×27 facts across PR-A + PR-B | **shipped** (PR-A #85, PR-B #86; foundation for Stage 7 Claude Code roundtrip; spec §4a) |
| 5     | Streaming output notifications         | NVDA reads `dir` line by line; spinner doesn't flood | **functional** as of PR #116 (pipeline reaches NVDA end-to-end). The verbose-readback issue — Stage 5's `renderRows` design announces the whole rendered screen on every emit — is the **first foundational architecture decision** addressed by the [May-2026 plan](PROJECT-PLAN-2026-05-09.md)'s Output framework cycle (Part 3); the original Stage 5 generic coalescer remains in place as the future Stream profile. |
| 6     | Keyboard input to PTY                  | PowerShell, vim, Ctrl+C all work; NVDA keys still work | **shipped** (Stage 6 PR-A parser arms + PR-B `KeyEncoding` + WPF input wiring + Job Object lifecycle). The minimal pass-through input pipeline is the **second foundational architecture decision** addressed by the [May-2026 plan](PROJECT-PLAN-2026-05-09.md)'s Input framework cycle (Part 4). |
| 5a    | Diagnostic logging surface             | File-based structured logging; per-session-per-day-folder layout; Issue #107 filename scheme; `Ctrl+Shift+L` open-logs hotkey; `Ctrl+Shift+;` copy-active-log hotkey; `FlushPending` barrier so the copy captures up-to-the-moment state | **shipped** (multi-PR cycle through #103 / #109 / #111 / #114 / #116 + Issue #107 + FlushPending; spec §5a). Enabled the post-Stage-5/6 streaming-pipeline-fix cycle by giving the maintainer a one-keystroke "paste log" diagnostic path. |
| 7     | Run Claude Code end-to-end             | Roundtrip prompt → response, NVDA reads it      | **shipped** (2026-05-03) across 11 sequenced PRs (PR-A through PR-K, #131-#143) plus PR-L #144 (doc-purpose audit) and PR-M #145 (coalescer cross-row spinner gate fix). Maintainer NVDA-validation green: cmd / PowerShell / Claude all spawn, stay alive, and announce correctly. Gap inventory (`docs/STAGE-7-ISSUES.md`) drove the post-Stage-7 substrate cycle (PRs #146-#184) + the Tier 1 SessionModel implementation cycle (PRs #185-#199). |
| 8     | Interactive list detection + UIA list  | Selection prompt announced as listbox           | pending — **subsumed** by the [May-2026 plan](PROJECT-PLAN-2026-05-09.md)'s Output framework cycle (Part 3 Stage F: Selection profile). List-detection becomes a profile-detection signal; UIA list exposure becomes a presentation-sink contract. |
| 9     | Earcons (NAudio) + color announcement  | Red plays alarm; Ctrl+Shift+M mutes             | pending — **subsumed** by the [May-2026 plan](PROJECT-PLAN-2026-05-09.md)'s Output framework cycle (Part 3 Stage G: earcons + colour-to-earcon mapping as a presentation sink the framework drives). Gates the `Ctrl+Shift+M` mute hotkey reservation. |
| 10    | Review mode + structured navigation    | `e` jumps to next error in scrollback           | pending — sequenced as **Part 5 (post-frameworks)** of the [May-2026 plan](PROJECT-PLAN-2026-05-09.md); first non-built-in consumer of the framework's semantic-event taxonomy. |
...
Per [`docs/PROJECT-PLAN-2026-05.md`](PROJECT-PLAN-2026-05-09.md), the
