...
| [`docs/CYCLE-52-R5-PLAYBOOK.md`](CYCLE-52-R5-PLAYBOOK.md) | Next / recovered session doing R5 (PowerShell adapter) + the pre-R5 pruning | **Start-here for all active work** (after SESSION-HANDOFF, before any R5 code) | Self-contained recovery brief: state, the standing cmd-announce FREEZE, R5a→R5d scope (file-anchored), the P1–P5 old-code pruning sequence, the battle-tested ops playbook. Closure-audit PR marks it HISTORICAL → `archive/cycle-closed/`. |
| [`docs/archive/cycle-closed/CYCLE-51-PLAYBOOK.md`](archive/cycle-closed/CYCLE-51-PLAYBOOK.md), [`CYCLE-49-PLAN.md`](archive/cycle-closed/CYCLE-49-PLAN.md), [`CYCLE-46-NEXT-STEPS.md`](archive/cycle-closed/CYCLE-46-NEXT-STEPS.md) | Historical reference only | Reverse-engineering a planning-vs-shipped delta | Shipped-cycle execution plans, HISTORICAL-bannered + archived (not link-checked at depth 3). Not start-here docs. |
...
| [`docs/IOCELL-SCHEMA.md`](IOCELL-SCHEMA.md) | Anyone touching IOCell extraction / persistence / the JSONL wire format | Before editing the serializer or round-trip reader; when bumping `schemaVersion` | Canonical decades-stable reference for the `IOCell` type (the unit of shell interaction, ADR 0004). In-memory shape (`IOCell` record + `IOCellPhase` DU; identity-at-creation contract; `CellSequence`); on-disk JSONL wire format at `schemaVersion 2` (locked key order, tagged-DU `phase`, int64 `cellSequence`, string-escape policy, one-way migration); round-trip discipline (`parseFromJsonl`, PR-W2). Supersedes SESSION-MODEL.md §"On-disk wire format". |
| [`docs/SESSION-MODEL.md`](SESSION-MODEL.md) | Anyone following a legacy link here | Tracing the pre-Cycle-51 substrate research design | **Stub (Cycle 51 PR-W).** The unit of interaction is now `IOCell`; wire format → IOCELL-SCHEMA.md; decision record → ADR 0004. Pre-pivot research-stage design (OSC 133 protocol, per-shell heuristic fallback, pathway-integration archaeology, 8 open questions) is preserved in git history. Cross-doc reframe of its ~13 referrers is the Cycle 51 PR-Z closure-audit's job. |
...
| [`docs/RELAUNCH-SPEC.md`](RELAUNCH-SPEC.md) | Anyone working the re-launch path | **FIRST for any new work** (supersedes ADR 0010's framing) | The audio-first multi-agent workspace specification: core canon (§0.1/§0.2), chunk-tree model (§5), self-voicing (§4.6), phased plan (§13). |
| [`docs/ENGINE-PHASE0.md`](ENGINE-PHASE0.md) | Maintainer running the Phase 0 engine | After Phase 0 ships / before the `P0-ENGINE-1` dogfood | Build + run + key map + the §13 acceptance walk for `Engine.Host`, judged on the self-voicing channel (no NVDA). |
| [`docs/engine/USER-GUIDE.md`](engine/USER-GUIDE.md) (+ `WORKFLOWS`, `KEYBOARD-REFERENCE`, `CONFIGURATION`, `TROUBLESHOOTING`) | Engine end users | First sessions / daily reference | The user-facing corpus for the audio-only computational notebook. |
| [`docs/engine/DEVELOPMENT-GUIDE.md`](engine/DEVELOPMENT-GUIDE.md) | Engine developers / maintainers | Before any engine change or field triage | Worked recipes per change class (verb / chunk kind / participant / bus consumer / config key), field debugging from dump + event log + session file, release checklist. |
| [`docs/engine/TEXTBOOK-00-OVERVIEW.md`](engine/TEXTBOOK-00-OVERVIEW.md) → `TEXTBOOK-10` | Anyone learning the engine internals | Deep orientation | Eleven chapters covering every module: data flow, the seven tested laws, and the why behind each design. |
