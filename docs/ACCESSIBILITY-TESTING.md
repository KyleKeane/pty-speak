...
| **52-ADR7-P2c** | **ADR 0007 Phase 2c — jump to last failed cell** (behavioural; **menu-only**, no accelerator). Installed preview (post the Phase 2c PR). Under **cmd**: **(1)** Fresh session, before any command: `Alt` → Output History → **Jump to Last Error**. **(2)** Run a command that *succeeds* (`echo ok` ⏎), then **Jump to Last Error** again. **(3)** Run a command that *fails* (`dir \\no\\such\\path` ⏎ — cmd sets a non-zero `errorlevel`), then a *successful* one (`echo done` ⏎). Now `Alt` → Output History → **Jump to Last Error**. **(4)** Run a *second* failing command (`type nonexistent.file` ⏎), then **Jump to Last Error** again. **(5)** From there, `Ctrl+Shift+Up`/`Down` a few times to confirm normal Manual nav still works from the jumped position. **(6)** `Ctrl+Shift+D`. | **(1)** & **(2)** Announce **"No failed command in history."** — nothing has exited non-zero yet. **(3)** The speech cursor jumps to and narrates the **failed `dir`** cell (its output — the error text), *not* the later successful `echo done`. **(4)** Jumps to and narrates the **`type nonexistent.file`** cell (the *newest* failure, not the older `dir`). **(5)** `Ctrl+Shift+Up`/`Down` move relative to the jumped cell (normal Manual nav unaffected). The menu item is present under Output History, keyboard-reachable, and announces **no** shortcut (menu-only). **Bundle checks:** one `ADR 0007 Phase 2c jump-to-last-error. Found={true|false} CellSeq={…}` per invocation — counts only, no command/output text. **Fail signals:** the item missing or announcing a phantom shortcut; jumping to a *successful* cell or the wrong (older) failure; "No failed command" after a real failure; normal `Ctrl+Shift+Up/Down` broken after a jump; command/output text in the bundle. |
