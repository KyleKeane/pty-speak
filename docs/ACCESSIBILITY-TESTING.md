...
| **47-13** | **Semantic-boundary markers in the document**: run two or three commands (e.g. `echo hi`, `dir`, `echo bye`). Press `NVDA+Numpad7` to move the review cursor to the top of the document. Press Down-Arrow line-by-line through the buffer. | The review cursor lands on `--- prompt ---` lines between commands. For shells emitting OSC 133 (claude), `--- input begins ---` / `--- output begins ---` / `--- output ends ---` are also navigable. For cmd, only `--- prompt ---` appears (cmd doesn't emit OSC 133). The marker lines are NOT visible on screen — only in the NVDA review-cursor document. |
...
| **47-15** | **No marker labels audible during auto-narrate**: in a fresh session, run `echo hi`. Listen for the verbatim string `"prompt"` or `"output"` in the spoken text. | NVDA narrates only `"hi"`. The audible path does NOT include `"--- prompt ---"`, `"--- output begins ---"`, etc. Those marker lines remain in the review-cursor document (47-13 still passes) but the auto-narrate path doesn't trigger NVDA to read DocumentRange. |
