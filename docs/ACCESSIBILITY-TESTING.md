...
| **52-ADR7-P3** | **ADR 0007 Phase 3 — rerun focused input** (behavioural; **menu-only**, no accelerator; two-step arm/confirm). Installed preview (post the Phase 3 PR). Under **cmd**: **(1)** Fresh session: `Alt` → Output History → **Rerun Focused Input** with nothing navigated. **(2)** Run `echo hello` ⏎. The cell is auto-followed (AutoDrive). `Alt` → Output History → **Rerun Focused Input** (1st time = arm). **(3)** Wait, listen to the full announce, then `Alt` → Output History → **Rerun Focused Input** a 2nd time (within 15 s) = confirm. **(4)** Arm once more, then wait **>15 s** and invoke again. **(5)** Arm on the `echo hello` cell, then `Ctrl+Shift+Up` to a *different* cell and invoke. **(6)** `Ctrl+Shift+D`. | **(1)** **"No focused cell. Navigate to a command cell first."** — nothing injected. **(2)** Announce **"Rerun command from cell N: echo hello? Select Rerun Focused Input again within 15 seconds to confirm."** — the shell does **not** run anything yet. **(3)** Announce **"Rerunning command from cell N: echo hello"** and the shell actually runs `echo hello` again (a new cell with fresh `hello` output is produced and auto-narrated). **(4)** The stale arm is ignored — invoking after the window just **re-arms** (speaks the "Rerun … ? … confirm" prompt again), it does **not** run. **(5)** Changing the focused cell invalidates the arm: the new invocation arms for the *new* cell (no cross-cell confirm). The menu item is under Output History, keyboard-reachable, announces **no** shortcut. **Bundle checks:** `ADR 0007 Phase 3 rerun-input ARMED. SourceCellSeq={…} CmdLen={…}` then `… CONFIRMED. …` — counts only, **no command text** in the log (the command is spoken via NVDA, never written to the bundle). **Fail signals:** the item missing or announcing a phantom shortcut; a *single* invocation running the command (confirm gate bypassed — risk!); the stale (>15 s) or cross-cell case running instead of re-arming; the command text appearing in the `Ctrl+Shift+D` bundle (logging-discipline breach); rerun not producing a fresh cell. |
