...
| **47-2** | Click `Simple Echo` (test 01). | NVDA announces "Test command inserted: test-01-echo. Press Enter to run." The script path is now in the cmd input buffer. Press Enter; the intro line, all three numbered lines (`Line 1 of 3.` → `Line 3 of 3.`), and the final "If you heard…" message audible; ready-prompt click at the end. (Pre-Cycle-49 wording was "all eight numbered lines"; script reshaped 2026-05-14 PR-H so the intro and final lines are explicitly counted as well.) |
...
| **47-17** | **Test-bracketed extractor — single run**: run test 01 (`Simple Echo`) once. Open Alt → Diagnostics → Extract → Test Run → Test 01 — Echo. | NVDA announces "ExtractTestEcho copied to clipboard: <N> bytes. Extract file at %LOCALAPPDATA%\\PtySpeak\\extracts\\extracttestecho-<ts>.txt." Pasting yields the three `"Line N of 3"` lines + the surrounding intro and final-message echo lines, bracketed by the extract-file header. (Pre-Cycle-49 the script printed eight numbered lines; reshaped 2026-05-14 PR-H.) No surrounding session prompts. Run test 01 again, fire the same extractor: clipboard now shows both runs with `--- test-01-echo run 1 of 2 ---` / `--- test-01-echo run 2 of 2 ---` dividers. Fire the extractor in a fresh session before running test 01: clipboard reads `"(no runs of test-01-echo found in the bundle)"`. |
...
| **49-1** | **Cycle 49 consolidated walk** (post-PR-A → PR-I, 2026-05-14). Run the corpus end-to-end: test 01 (`Simple Echo`), test 02 (`Text Input`), `echo hi` twice in a row, Up/Down arrow at the cmd prompt with non-empty history, `Alt` → arrow through the top-level menu, `Ctrl+Shift+Up/Down/End` for manual SpeechCursor. | (PR-A) Manual SpeechCursor lands on one stop per audible chunk — no parking on blank entries. (PR-B → PR-F → PR-G) Test 02 sub-prompt announces only `Enter your name:` (last line of accumulator); after typing a name + Enter, only `Hello, NAME!...` + end-marker narrate (no re-read of the preamble or duplicate suppression). (PR-C) After any tuple-final, NVDA's review cursor (`NVDA+Numpad7` to jump to top) immediately sees the freshest content without needing another command first. (PR-D) `Ctrl+Shift+Up` past command output lands on the prompt as its own announce (e.g. `current>` or similar, `FinalDirOnly`-trimmed). (PR-G) Running `echo hi` twice in a row: BOTH runs narrate. (PR-H) Test 01 narrates the intro, three `Line N of 3.` lines, and the final message — review cursor finds each by literal string. (PR-I) Up arrow at cmd prompt narrates the recalled previous command via the `pty-speak.input-assistant` activity ID; rapid Up spam coalesces to one final announce; empty history is silent (no blank-line announce). Menu narration: `Alt` opens the top menu and arrow keys narrate each item (no regression from the duplicate-suppression bug PR-G fixed). |
| **52-1** | **Cycle 52 R1–R4 foundation dogfood** (post-`cbf8d48`, #347–#357, 2026-05-16). Installed preview (NOT a dev `dotnet run` — see §"Dogfood operations"); restart NVDA if the UIA-notification wedge bites. Run: (a) `echo hi` + Enter; (b) the corpus test 01 / test 02 / test 04 (single-key Y/N); (c) **sustained volume — run ≥ 12 commands in a row** (mix of `echo`, `dir`, `cd`); (d) Up/Down history-recall then Enter; (e) `Ctrl+Shift+1`→cmd, `Ctrl+Shift+3`→claude, `Ctrl+Shift+1`→cmd (shell switches); (f) `Ctrl+Shift+D` bundle. | **R1 (behaviour-identical baseline):** test 01 / test 02 narrate as pre-Cycle-52 (intro, `Line N of 3.`, final message; sub-prompt question once). **R2 (cmd OSC-133):** clean command/output split, **no visible escape-sequence leakage** in the prompt; bundle shows `extractIOCell arm. Arm=CmdOscAB` and `R2 cmd OSC-133 prompt injection applied (startup)`. **R3a (precedence):** bundle shows the one-time `R3a precedence: first OSC-133 boundary observed` line; after it, no heuristic-driven mis-announces. **R3b + KI-R2-1 (the key check):** under the ≥ 12-command volume run, the echo output + next input path **must NOT** leak into the announce and **no path-suppression toggle should be needed** (KI-R2-1 must be gone — it was the PR-X `lastEnterSeq`/`EndsWith` path R3b deleted). Bundle shows `R3b tuple-final announce (sealed IOCell OutputText)`. **R3b sub-prompt watch:** on test 02 / test 04 the sub-prompt question must be spoken **once**, not twice (double-speech ⇒ the accepted R3b re-derivation risk materialised → R3b-followup). **Banner:** the cmd→claude→cmd switches must each still **read the shell banner** (PR-AA/AC preserved). **R4a:** bundle greps for the heuristic detector now use category `Terminal.Shell.HeuristicPromptDetector` (the old `Terminal.Core.…` string returns nothing). **Pre-existing (not a blocker):** backspace-to-empty narrating the full input path — confirm it still classifies as pre-existing / R2-backlog, not a new regression. |
| **52-R3c** | **R3c spoken-watermark re-dogfood** (post the R3c PR; the 2026-05-16 foundation dogfood found #1 banner silent + #2 interactive double-speech). Installed preview; restart NVDA if the wedge bites. Run: (a) **fresh launch** — listen on first focus; (b) test 02 (`set /p`) + test 04 (single-key Y/N) + any interactive test; (c) `Ctrl+Shift+1`→cmd / `Ctrl+Shift+3`→claude / `Ctrl+Shift+1`→cmd; (d) the ≥ 12-command volume run again; (e) `Ctrl+Shift+D` bundle. | **#1 banner — must now READ on fresh launch** (the shell banner, not "Terminal, edit, blank") AND on each shell switch. Bundle shows `R3c banner announce (un-spoken pre-prompt gap). FromSeq=…`. **#2 — sub-prompt question spoken EXACTLY ONCE.** On test 02: the question/preamble is announced in real-time when it appears; after typing the answer + Enter, **only the post-response output** narrates — the question/preamble is **NOT** re-read. Same for test 04. Bundle shows `R3c tuple-final announce (un-spoken Seq gap). FromSeq=…` (FromSeq > the sub-prompt announce's Seq). **KI-R2-1 still gone** (the ≥ 12-command run: no echo/next-path leak — R3c keeps it fixed; the watermark is the *settled* `lastAnnouncedSeq`, not the racy `lastEnterSeq`). **Regression sweep:** plain `echo hi` still narrates its output once; `Ctrl+Shift+H` still works (and now leads with `Version …` per #5). If #2 still double-speaks, capture the bundle's `R3c tuple-final announce … FromSeq=` vs the sub-prompt announce Seq — that localises whether the watermark advanced as expected. |
| **52-R3c-multi** | **R3c multi-interrupt watermark-composition** (new `test-09-multi-interrupt`; maintainer-requested 2026-05-16 — verifies the watermark composes across MULTIPLE incremental announces in one cell, not just one sub-prompt). `Diagnostics → CMD Interaction Tests → Multi-Interrupt (R3c)`, Enter to run. Answer Y or N at each of the two prompts. | The following are spoken in **this exact sequence, each exactly ONCE, with no re-read of any earlier segment**: (1) "SECTION ONE …"; (2) "INTERRUPTION ONE: press Y or N" (real-time); (3) [you answer]; (4) "SECTION TWO …" — **alone**, NOT prefixed by a re-read of section one or interruption one; (5) "INTERRUPTION TWO: press Y or N" (real-time); (6) [you answer]; (7) "SECTION THREE …" — alone. **Fail signal:** any earlier segment heard a second time after a later answer ⇒ the watermark did not advance across the second incremental announce (the composition case R3c must hold for R5/R6). Bundle: successive `R3c tuple-final announce … FromSeq=` lines with strictly increasing `FromSeq`. |
| **52-R3d** | **R3d plain-command echo regression fix** (the SHA-confirmed bundle for build `09321e7` proved R3c re-spoke the typed command on plain commands). Type a few plain commands by hand: `echo hi` ⏎, `dir` ⏎, `echo testing one two three` ⏎. Listen to each announce. Then `Ctrl+Shift+D`. | Each plain command announces **only its output** — `echo hi` → "hi" (NOT "echo hi hi"); `echo testing one two three` → "testing one two three" (NOT the command echoed first); `dir` → the listing only. **Fail signal:** the typed command is voiced before its output. Bundle shows `R3d tuple-final announce (clean output, watermark-intersected). CommandEnterSeq=… LastAnnouncedSeq=… FromSeq=…` where `FromSeq = max(CommandEnterSeq, LastAnnouncedSeq)` and the announced body is output-only. (#2 sub-prompt one-read and `52-R3c-multi` must still pass — R3d preserves them; re-run those too.) Note: the **synthetic diagnostic battery** (`Ctrl+Shift+D`'s own `echo PtySpeakDiag…`) does not set `commandEnterSeq`, so its bundle lines may still show the command echo — that is a harness-only artifact, NOT the real typed-command path; judge R3d by hand-typed commands. |
| **52-R3e** | **R3e single-key sub-prompt re-arms Executing — multi-sub-prompt fix** (the SHA-confirmed bundle for build `62fdc2d` proved `test-09` left the 2nd sub-prompt unannounced + mis-tagged resumed output `UserInputEcho` after the first single-key answer). `Diagnostics → CMD Interaction Tests → Multi-Interrupt (R3c)`, Enter to run. Answer Y or N at **each** of the two prompts (single keystroke — `choice`, no Enter). Then `Ctrl+Shift+D`. | Spoken in **this exact sequence, each exactly ONCE**: (1) "SECTION ONE …"; (2) "INTERRUPTION ONE: press Y or N" (real-time); (3) [single-key answer]; (4) "SECTION TWO …" — **alone**; (5) "INTERRUPTION TWO: press Y or N" (real-time — **this is the R3e fix; pre-R3e it was SILENT**); (6) [single-key answer]; (7) "SECTION THREE …" — alone. **Fail signal:** INTERRUPTION TWO not announced, OR section-two/three voiced as the typed answer key (mis-tag), OR any earlier segment re-read. Bundle shows the `ShellInteraction transition: Composing --[SingleKeySubmitted]--> Executing` line after each single-key answer, then `Executing --[SubPromptIdle(...)]--> Composing` for the 2nd prompt; successive `R3d tuple-final announce …` lines with strictly increasing `FromSeq`. (52-R3c-multi is now this same test under R3e — the single-key path is the real defect; supersede the pre-R3e expectation.) |
| **52-R5b** | **✅ PASSED 2026-05-16 (basic interaction; #1 R5 risk resolved positive).** Maintainer dogfood: switching to PowerShell works; the "PSReadLine disabled … screen reader" warning reads (expected); `echo hi` ⏎ → NVDA speaks `hi` (clean command/output split — works as expected). The `prompt`-function path emits OSC-133 under NVDA despite PSReadLine being disabled; the `;C`-unreachable consequence is the accepted final design (ADR 0005 §3 R5 status note). Deeper PS-specific scenarios deferred to the R6 `Diagnostics → PowerShell Interaction Tests` submenu (ADR 0006 §"Deferred to R6+" item 7). Original procedure retained below for re-runs. — **R5b PowerShell adapter — OSC-133 emission (the #1 R5 risk is empirical: PSReadLine-disabled-under-screen-reader).** Installed preview. From cmd, `Ctrl+Shift+2` → PowerShell. Listen on the first PowerShell prompt. Run by hand: `echo hello` ⏎; `Get-ChildItem` ⏎; a command that fails (e.g. `cmddoesnotexist` ⏎) then `echo ok` ⏎. Then `Ctrl+Shift+1`→cmd, `Ctrl+Shift+2`→PowerShell again (hot-switch round-trip). Then `Ctrl+Shift+D`. | **PowerShell spawns and is usable** under NVDA (PowerShell will print its "PSReadLine disabled … screen reader" warning — expected, not a failure; the `prompt` function is independent of PSReadLine). **Clean command/output split** like cmd: `echo hello` → "hello" only (no command echo, no next-prompt-path bleed — same `CmdOscAB` arm as cmd). **Real exit code:** the failing command's cell carries a non-zero `ExitCode` (cmd always had `None`; PowerShell's is `Some <code>`). **Hot-switch round-trip:** cmd↔PowerShell each read their banner + behave per their shell; the seam is transparent. **Bundle checks:** `R5b PowerShell OSC-133 prompt injection applied (startup|shell-switch)` appears for PowerShell (and the cmd `R2 …` line still appears for cmd, unchanged); `extractIOCell arm. Arm=CmdOscAB` for PowerShell cells; the integrated command line is `powershell.exe -NoExit -EncodedCommand <base64>` (decode UTF-16LE → the `prompt` function). **Fail signals:** PowerShell doesn't spawn / hangs; OSC sequences leak visibly into the prompt (escape-char gibberish read aloud) ⇒ the `Osc133InitScript` needs a contained tweak; command/output bleed like the deferred cmd cases; no exit code on a known-failing command. **#1-risk readout:** if OSC-133 does *not* emit at all under the screen reader (no `Arm=CmdOscAB` for PS, prompt unstructured), report it — that decides whether the `prompt`-only baseline holds or R5 needs a different mechanism. |
| **52-R4c** | **R4c cmd deferred `;D` (boundary-only CommandFinished) — pre-R5 transport completion** (folds into the R1–R4 foundation dogfood; same installed preview, one NVDA pass). On a **fresh launch**: listen on first focus. Then run, by hand: `echo hi` ⏎; `dir` ⏎; `echo one two three` ⏎ (≥ 3 plain commands). Then corpus test 02 (`set /p`) + test 04 (single-key Y/N). Then `Ctrl+Shift+Up/Down/End` manual SpeechCursor over the session. Then `Ctrl+Shift+D`. | **No regression vs 52-R3d/R3e:** each plain command still announces **output only** (`echo hi` → "hi", not "echo hi hi"); test 02/04 sub-prompt question spoken **exactly once**. **Fresh-launch sanity:** the first prompt does **not** produce a spurious "end output" / blank announce, and manual SpeechCursor at session start has **no stray silent stop before the first command** (the leading-`;D` gate). **Manual nav:** after a command completes, `Ctrl+Shift+Up` still finds the output; the end-of-output boundary is detectable (now a *real* `CommandFinished` marker, correctly placed *after* the output — pre-R4c it was the misplaced synthetic one). **Bundle checks:** `extractIOCell arm. Arm=CmdOscAB` still present; `R2 + R4c cmd OSC-133 prompt injection` (or the existing R2 injection line) shows the integrated command line containing `133;D` then `133;A` then `133;B` and **no** `133;D;` (boundary-only — no exit code) and **no** `%errorlevel%`; on fresh launch a `R4c cmd CommandFinished suppressed (no cell finalised; leading or drop-on-None ;D)` line appears **once** (the leading `;D`) and **not** on every subsequent command. **Fail signals:** trailing next-prompt path leaks into the announced output or the persisted `OutputText`; a stray "end output" at launch; the typed command voiced before its output; the sub-prompt question re-read. |
| **52-R6a** | **✅ PASSED 2026-05-16.** Maintainer dogfood (post-`3ca274f` preview): the progress loop announced **each output line as it appeared** during the long-running command (cmd + PowerShell); `echo` and claude unaffected. **Two observations, neither a blocker:** (1) on the *first few* echoes the next-prompt path was heard after the output, then it **stopped and could not be reproduced even across a restart** — classed **transient watch-item**, not chased (no reliable repro to localise; if it recurs, capture `Ctrl+Shift+D` at the moment and grep `R3d tuple-final`/`R6a progress` to localise). (2) a progress chunk **interrupts** whatever is currently speaking — **expected v1 behaviour** (the `ActivityIds.output` + `MostRecent` supersede is by design); smoother queue-rather-than-interrupt is a **deferred R6+ refinement** (ADR 0006 §"Deferred to R6+" item 9), explicitly *not* in scope now. Maintainer: "good to progress to the next stage." **Gate cleared → R6b unblocked.** Original procedure retained below for re-runs. — **R6a hybrid progress streaming — long-running command not silent until it seals** (ADR 0006 R6; first R6 audible-behaviour change; gates R6b). Installed preview; restart NVDA if the UIA-notification wedge bites. Under **cmd** run, by hand, a command whose output trickles in over several seconds so the 350 ms idle-flush fires *mid-execution*: `ping -n 8 127.0.0.1` ⏎ (one reply line ≈ every second). Listen during the ~8 s before it finishes. Then a plain fast command `echo hi` ⏎ (must be unaffected — seals before any flush). Then `Ctrl+Shift+2` → **PowerShell**, repeat `ping -n 8 127.0.0.1` ⏎ + `echo hi` ⏎. Then `Ctrl+Shift+3` → **claude**, type a prompt that streams a multi-second reply. Then `Ctrl+Shift+D`. | **cmd / PowerShell (`TupleFinalOnly`):** the ping replies are **announced progressively while the command is still running** — NVDA is no longer silent for the whole ~8 s and only speaking at the end. When the command finishes, the final seal speaks **only the un-spoken remainder** — **no re-read** of reply lines already voiced (the R3c/R3e watermark composes: each flush advances `lastAnnouncedSeq`, the `;D` seal slices from there). **Plain fast command unaffected:** `echo hi` → "hi" once, no progress chunk (it seals before the 350 ms flush; `echo hi` ≠ "echo hi hi"). **claude unaffected:** its `IdleFlushMs = None` ⇒ the R6a arm never runs; streaming reply narrates exactly as pre-R6a (no new mid-stream progress chunks, no double-talk). **Bundle checks:** under cmd/PowerShell ping, one or more `R6a progress announce (Executing idle-flush). CommandEnterSeq=… LastAnnouncedSeq=… FromSeq=… Len=…` lines with **strictly increasing `FromSeq`** (= `max(CommandEnterSeq, LastAnnouncedSeq)`), then the existing `R3d tuple-final announce …` seal line with `FromSeq` > the last progress `FromSeq`. **No** `R6a progress announce` line for `echo hi` or for the claude session. **Fail signals:** any earlier ping line heard a second time after a later chunk or at the seal (watermark didn't advance ⇒ R6a regressed the R3c-multi composition); the typed command voiced (`ping -n 8 127.0.0.1` spoken before its replies); progress chunks fire while *Composing* at a prompt (gate leaked — should be Executing-only); claude or an `Off`/`LineByLine`-policy shell emitting `R6a progress announce`; sub-prompt question (test 02/04) re-read — re-run 52-R3d/R3e/R4c to confirm those still pass (R6a must preserve them). |
| **52-R6b** | **✅ PASSED 2026-05-16.** Maintainer dogfood: "This works." Default unchanged-silent (Part 1), Full-On-Change behaved (Part 2), shell-switch reset behaved (Part 3). **One known limitation, NOT a blocker and explicitly NOT addressed now (maintainer's call):** announcing the prompt path **interrupts** an in-progress read of the previous command's output — parked as ADR 0006 §"Deferred to R6+" item 10. The maintainer also requested three additional on-change modes in this dogfood; those shipped as **R6b-followup** (see the `52-R6b-followup` row) — `52-R6b`'s gate is cleared → **R6c unblocked**. Original procedure retained below for re-runs. — **R6b prompt-path verbosity — new "Full On Directory Change" mode** (ADR 0006 R6; default unchanged — this is an opt-in mode; gates R6c). Installed preview (post the R6b PR). **Part 1 — default unchanged (regression):** fresh launch under cmd; run `echo hi` ⏎, `dir` ⏎, `cd ..` ⏎ a few times **without** touching the menu. **Part 2 — select the new mode:** press `Alt` → Prompt Path → **Full On Change** (Enter). Then under cmd: `echo hi` ⏎ (same dir), `echo two` ⏎ (same dir), `cd ..` ⏎, `echo three` ⏎ (new dir), `cd` back ⏎. **Part 3 — shell-switch:** `Ctrl+Shift+2` → PowerShell (listen on first prompt), run `echo ps` ⏎ then `echo ps2` ⏎ (same dir), `Ctrl+Shift+1` → cmd. **Part 4:** `Ctrl+Shift+D`. | **Part 1 (default = Suppress, unchanged):** auto-drive is **silent at every prompt** exactly as before — `echo hi` → "hi" only, no path spoken at the prompt; `Ctrl+Shift+Up/Down` still surfaces the prompt in review (manual-nav unchanged). No behaviour change vs pre-R6b. **Part 2 (Full On Change):** selecting it announces the cue **"Prompt path full on directory change."** Then: the **first** prompt after selecting narrates the **full path** (e.g. `C:\Users\…\X>`); running `echo hi` then `echo two` **in the same directory** → the next prompts narrate **final-dir-only** (e.g. `X>`), NOT the full path again; after `cd ..` the **changed** prompt narrates the **full path** again; back to same-dir commands → final-dir-only again. **Part 3 (shell-switch reset):** the **first** PowerShell prompt after the switch narrates the **full path** (reset cleared the watermark — orientation on landing in the new shell), then same-dir `echo ps`/`echo ps2` → final-dir-only; switching back to cmd → its first prompt full again. **Bundle checks:** `R6b prompt-path on-change resolve. Changed={Changed} Resolved={Full|FinalDirOnly} KeyLen={…}` lines — `Changed=true Resolved=Full` on the first prompt + each `cd`/shell-switch, `Changed=false Resolved=FinalDirOnly` on same-dir repeats. **No** `R6b prompt-path on-change resolve` lines at all during Part 1 (Suppress ⇒ the on-change arm never runs). **Fail signals:** Part 1 auto-drive suddenly speaking prompts (default leaked from Suppress); same-dir prompts narrating the full path (change-detection not seeing "unchanged"); a `cd` / shell-switch narrating only the final dir (change/reset not detected); the cue not announced on selection; the menu item missing under `Alt → Prompt Path`. |
| **52-R6b-followup** | **R6b-followup — three additional on-change prompt-path modes** (ADR 0006 R6; default still `Suppress` unchanged; rides the `52-R6b` gate as a followup, not a new gate). Installed preview (post the R6b-followup PR). Under **cmd**, press `Alt` → Prompt Path and for **each** of the three new items run: `echo a` ⏎, `echo b` ⏎ (same dir ×2), `cd ..` ⏎, `echo c` ⏎ (new dir), `cd` back ⏎. The three items: **(A) "Final Dir On Change, Full When Same"**, **(B) "Full On Change, Silent When Same"**, **(C) "Final Dir On Change, Silent When Same"**. Also confirm the original **"Full On Directory Change"** still behaves (regression). Then `Ctrl+Shift+2` → PowerShell, pick (B), run `echo ps` ⏎ `echo ps2` ⏎. Then `Ctrl+Shift+D`. | Selecting each announces its cue: (A) **"Prompt path final directory on change, full when unchanged."** (B) **"Prompt path full on change, silent when unchanged."** (C) **"Prompt path final directory on change, silent when unchanged."** Behaviour: **(A)** first/`cd` prompt → **final-dir-only** (`X>`), same-dir repeats → **full path** (the mirror of Full-On-Change). **(B)** first/`cd` prompt → **full path**, same-dir repeats → **silent** (no prompt spoken at all). **(C)** first/`cd` prompt → **final-dir-only**, same-dir repeats → **silent**. **Regression:** the original "Full On Directory Change" still = full on change / final-dir when unchanged. **Shell-switch:** picking (B) then switching shells → the first prompt in the new shell is "changed" (full path), then same-dir → silent. **Bundle checks:** `R6b prompt-path on-change resolve. Mode={…} Changed={…} Resolved={Full|FinalDirOnly|Suppress} KeyLen={…}` — `Resolved=Suppress` on the unchanged repeats under (B)/(C); `Mode=` reflects the selected mode. **Fail signals:** a cue mismatched to the selected item; (B)/(C) speaking anything on a same-dir repeat (silent-when-unchanged not honoured); (A) speaking full on change instead of final-dir; any mode garbling the prompt; an item missing under `Alt → Prompt Path`. |
| **52-ADR7-P0** | **ADR 0007 Phase 0 — delete the dead SpeechCursor Seq engine** (net-subtractive; no audible behaviour change; **regression-only**). Installed preview (post the Phase 0 PR). Under **cmd**: run `echo one` ⏎, `dir` ⏎, `echo two` ⏎ so there is history; listen to the normal AutoDrive narration. Then press `Ctrl+Shift+Up` a few times, `Ctrl+Shift+Down` a few times, `Ctrl+Shift+End` — review-navigate the cell history. Then `Ctrl+Shift+2` → PowerShell, repeat briefly. Then `Ctrl+Shift+D`. | **No change vs. pre-Phase-0.** AutoDrive narrates command output exactly as before; `Ctrl+Shift+Up/Down/End` still step through the sealed-cell transcript (command + output cells) and speak each; shell-switch still resets the cursor; selection-suspend unaffected. **Fail signals:** any cell-navigation key now silent / erroring / landing differently; AutoDrive narration changed; a crash or missing announce on shell-switch. (This phase deleted only code with zero production callers; any audible difference is a regression to investigate, not expected.) |
| **52-ADR7-P1** | **ADR 0007 Phase 1 — typed `CellTranscript`** (pure refactor; **byte-identical narration**; **regression-only**, same surface as `52-ADR7-P0`). Installed preview (post the Phase 1 PR). Run the **exact `52-ADR7-P0` procedure** (cmd + PowerShell history, `Ctrl+Shift+Up/Down/End` navigation, shell-switch, `Ctrl+Shift+D`). | **No change vs. pre-Phase-1.** Command + output cells navigate and speak identically (the transcript now carries typed identity/kind internally but the narrated text + activity id are projected unchanged); shell-switch reset + selection-suspend unaffected. The typed `CellView` shape is pinned by the unit suite, not audible here. **Fail signals:** any audible difference at all vs. `52-ADR7-P0` (Phase 1 must be transparent — a difference means the projection or append path regressed). |
| **52-ADR7-P2a** | **✅ FEATURE PASSED 2026-05-17.** Maintainer dogfood: the `Ctrl+Shift+C` hotkey and the Output History → Copy Focused Cell menu item both work. **A pre-existing substrate defect was surfaced (not introduced):** editing the command line before Enter (backspace/reflow) desyncs command/output cell pairing by one (phantom input cell) — recorded + decided ("continue phases, track it, no speculative patch") in ADR 0006 §"Deferred to R6+" item 1 "Tracked variant — command-line-edit input/output desync"; the navigable history made it deterministically diagnosable (ADR 0007 D6 premise validated). Phase 2a's gate is **cleared**; the desync gates nothing in the ADR 0007 nav/ops layer (it's upstream `extractIOCell`). Original procedure retained below for re-runs. — **ADR 0007 Phase 2a — copy the focused cell (`Ctrl+Shift+C`)** (first **behavioural** phase — real dogfood, not regression-only). Installed preview (post the Phase 2a PR). Under **cmd**: run `echo one` ⏎, `dir` ⏎, `echo two` ⏎. **(1)** With the cursor unparked (fresh, no `Ctrl+Shift+Up` yet) press **`Ctrl+Shift+C`**. **(2)** Press `Ctrl+Shift+Up` once or twice to focus a cell (you'll hear the cell narrated), then press **`Ctrl+Shift+C`**; switch to another window and paste (`Ctrl+V`) into Notepad to inspect. **(3)** Press `Ctrl+Shift+Up`/`Down` to focus a *different* cell, `Ctrl+Shift+C`, paste again. **(4)** Open the menu (`Alt`) → Output History → confirm a **"Copy Focused Cell to Clipboard"** item is present and announces its `Ctrl+Shift+C` shortcut. **(5)** `Ctrl+Shift+D`. | **(1)** No cell focused ⇒ announce **"No focused cell. Press Ctrl+Shift+Up or Down to focus a cell first."** — nothing written to the clipboard. **(2)** Announce **"Cell N copied to clipboard, B bytes."**; the pasted text is that cell's **Command:** line and **Output:** block (the whole cell, even though only one item was the parked one). **(3)** The paste reflects the *newly* focused cell, not the previous one. **(4)** The menu item exists, is reachable, and NVDA reads "Ctrl+Shift+C" as its shortcut; invoking it does the same as the key. **Bundle checks:** one `ADR 0007 Phase 2a focused-cell copy. CellSeq={…} CmdLen={…} OutLen={…} Bytes={…}` line per successful copy (counts only — no command/output text in the log). **Fail signals:** `Ctrl+Shift+C` does nothing / is swallowed by the terminal (keyboard-filter ordering regressed — it must reach the InputBinding); the no-focus case writes to the clipboard or is silent; the paste contains the wrong cell or only one side when both exist; the menu item missing or shortcut not announced; the command/output text appearing in the `Ctrl+Shift+D` bundle (logging-discipline breach). **⚠ Precondition correction (2026-05-17, maintainer dogfood):** step (1)'s no-focus guard is **only reachable on a fresh session BEFORE any command has completed** (or briefly after a shell hot-switch, before the next command). In the default **AutoDrive** mode `SpeechCursor.appendCell` auto-advances the cursor to the newest cell after *every* command (ADR 0007 D3 live-trickle — `SpeechCursor.fs:674-675`: `if state.Mode = AutoDrive then state.CellPos <- Count-1`). So after `echo one`/`dir`/`echo two`, `Ctrl+Shift+C` copies the **latest** cell with **no** `Ctrl+Shift+Up` — this is correct, intended behaviour, not a bug. The "unparked, no focus" wording in (1) above was a wrong test narrative; the **feature PASS stands** (the maintainer's run exercised the auto-followed-latest path and it worked). To exercise the no-focus guard, run the menu item as the *very first action* in a fresh session. |
| **52-ADR7-P2b** | **✅ FEATURE PASSED 2026-05-17.** Maintainer dogfood: output-only copy works and Manual-nav focus-hold works (the op binds to the navigated cell). **A pre-existing upstream defect was surfaced (not introduced):** a clean `echo hi` captures only `echo` as the command (whitespace-delimited argument lost) — recorded as **ADR 0006 §"Deferred to R6+" item 1 "Tracked variant 2 — command argument truncation on a clean command"**; same root-cause family as variant 1 (cmd command text reconstructed from a `commandEnterSeq`-bounded `ContentHistory` slice, not a real marker), **not a Phase 2b regression** (the copy faithfully copied an upstream-truncated command). Phase 2b's gate is **cleared**; the truncation gates nothing in the ADR 0007 nav/ops layer. Original procedure retained below for re-runs. — **ADR 0007 Phase 2b — copy one side of the focused cell** (behavioural; **menu-only**, no accelerator). Installed preview (post the Phase 2b PR). **Mode note:** default is **AutoDrive**, which auto-follows the latest cell after each command (ADR 0007 D3, `SpeechCursor.fs:674-675`); the no-focus guard is therefore reachable only with an *empty* transcript. **(1) No-focus guard — do FIRST, before any command:** fresh session, cmd, open `Alt` → Output History → **Copy Focused Cell Command**. **(2)** Now run `echo one` ⏎, `dir` ⏎. Directly (no `Ctrl+Shift+Up`) menu → **Copy Focused Cell Command**; switch to Notepad, `Ctrl+V`. **(3)** Same (auto-followed `dir`) cell, menu → **Copy Focused Cell Output**; paste again. **(4)** Press `Ctrl+Shift+Up` until you hear the **`echo one`** cell (Manual nav), menu → **Copy Focused Cell Command**; paste. **(5)** Focus a cell whose command (or output) is empty/whitespace if you can produce one, try the matching item. **(6)** `Ctrl+Shift+D`. | **(1)** Empty transcript ⇒ **"No focused cell. Press Ctrl+Shift+Up or Down to focus a cell first."**, nothing copied. **(2)** **"Cell N command copied to clipboard, B bytes."** with **no** prior navigation (AutoDrive auto-followed `dir`); the paste is *only* the `dir` command line (no output, no "Command:"/"Output:" labels). **(3)** **"Cell N output copied to clipboard, B bytes."**; the paste is *only* `dir`'s output. **(4)** The paste is **`echo one`** (the *navigated* older cell, not the latest) — confirms the op binds to the Manual cursor and Manual nav holds focus. **(5)** **"The focused cell has no command."** / **"…no output."** as appropriate; nothing copied. Both menu items are present under Output History, reachable by keyboard, and have **no** shortcut announced (menu-only by design). **Bundle checks:** one `ADR 0007 Phase 2b focused-cell side copy. Side={command|output} CellSeq={…} Len={…} Bytes={…}` per successful copy — counts only, no text. **Fail signals:** the item missing or announcing a phantom shortcut; the command variant pasting the output (or vice-versa); step (2) requiring navigation before it copies (AutoDrive auto-follow regressed); the empty-transcript / empty-side cases copying anything or going silent; command/output text in the bundle. |
| **52-ADR7-P2c** | **✅ FEATURE PASSED 2026-05-17 (PowerShell, external-process exit).** Maintainer dogfood: under `Ctrl+Shift+H`-confirmed **PowerShell**, `cmd /c exit 7` ⏎ then Output History → **Jump to Last Error** correctly jumps to and narrates that failed cell. **Scope clarified (this is what exit-code failure detection can see):** a non-zero exit code is only available when the shell *transports* one — **PowerShell** emits `;D;$LASTEXITCODE` for **external processes only** (a failing PowerShell *cmdlet* such as `dir`/`type` = `Get-ChildItem`/`Get-Content` does **not** set `$LASTEXITCODE`, so it is *not* an exit-code failure); **cmd** emits a bare `;D` with **no exit code at all** (documented transport limitation, `CmdAdapter.fs:52-65`, maintainer decision 2026-05-16 — clink/doskey out of scope), so under cmd Jump to Last Error can never find a failure. The Phase 2c follow-up replaces the misleading cmd "No failed command in history." with **"cmd does not report command exit codes; Jump to Last Error needs PowerShell."** (shell-type-gated; parses nothing → not a FREEZE-zone announce heuristic). The maintainer-observed boundary confusion during the test (inaccurate cell boundaries / prompt-path bleeding into output / non-sequential history) is the **already-tracked** ADR 0006 §"Deferred to R6+" item 1 variants 1 & 2 — *not* a Phase 2c bug. Phase 2c's gate is **cleared**. Corrected re-run procedure below. — **ADR 0007 Phase 2c — jump to last failed cell** (behavioural; **menu-only**, no accelerator). **Under PowerShell** (`Ctrl+Shift+2`; confirm via `Ctrl+Shift+H`): **(1)** Fresh session, before any command: `Alt` → Output History → **Jump to Last Error**. **(2)** Run a *successful* external command (`cmd /c exit 0` ⏎), then **Jump to Last Error** again. **(3)** Run a *failing* external command (`cmd /c exit 7` ⏎), then a successful one (`cmd /c exit 0` ⏎). Now **Jump to Last Error**. **(4)** Run a second failing external command (`cmd /c exit 3` ⏎), then **Jump to Last Error** again. **(5)** `Ctrl+Shift+Up`/`Down` to confirm normal Manual nav still works from the jumped position. **(6)** Under **cmd** (`Ctrl+Shift+1`): run any failing command, then **Jump to Last Error**. **(7)** `Ctrl+Shift+D`. | **(1)** & **(2)** **"No failed command in history."** (PowerShell; nothing has exited non-zero). **(3)** Jumps to and narrates the **`cmd /c exit 7`** cell, *not* the later successful one. **(4)** Jumps to the **`cmd /c exit 3`** cell (the *newest* failure, not the older `exit 7`). **(5)** Normal Manual nav unaffected by the jump. **(6)** Under cmd: **"cmd does not report command exit codes; Jump to Last Error needs PowerShell."** (honest capability message — *not* the old misleading "No failed command" line). The menu item is present under Output History, keyboard-reachable, announces **no** shortcut. **Bundle checks:** one `ADR 0007 Phase 2c jump-to-last-error. Found={true|false} Shell={…}` per invocation — counts only, no command/output text. **Fail signals:** the item missing or announcing a phantom shortcut; jumping to a *successful* cell or the wrong (older) failure under PowerShell; the old "No failed command in history." still spoken under cmd; normal `Ctrl+Shift+Up/Down` broken after a jump; command/output text in the bundle. |
| **52-ADR7-P3** | **✅ PASSED 2026-05-17 (insert + no-auto-run); ⚠️ one tracked-deferred known gap (line-clear).** Maintainer dogfood: rerun-focused-input and the diagnostic test insertion **insert the command correctly at the prompt and do NOT auto-run** (the user's Enter runs it) — the core behaviour passes. **Known gap (tracked, deferred, NOT a fresh fail):** the intended Esc line-clear does **not** clear pre-typed input — text inserts at the cursor but prior junk on the line is left intact. Pre-existing since Cycle 47; root-cause hypothesis (lone-ESC-under-ConPTY not delivered as a clear-line keypress; also cmd-cooked-mode vs PSReadLine shell-specific) + the maintainer's track-and-defer decision recorded in ADR 0007 §"Known issue surfaced" + the `insertAtPromptClearingLine` code comment + CHANGELOG. Do not re-fail P3 on the clear; it is logged. — The two-step arm/confirm was dropped (conservative reading of the ADR "confirm gesture" open decision; maintainer resolved it for the simple flow). **Behaviour:** Rerun Focused Input inserts the focused cell's command at the prompt (intended-but-currently-ineffective line-clear), **does NOT auto-run**; user presses Enter. Same `insertAtPromptClearingLine` path the diagnostic test-script insertion uses — one shared primitive. — **ADR 0007 Phase 3 — rerun focused input** (behavioural; **menu-only**, no accelerator). Under **cmd**: **(1)** Fresh session, nothing navigated: `Alt` → Output History → **Rerun Focused Input**. **(2)** Run `echo hello` ⏎ (cell auto-followed). Type some junk at the prompt (e.g. `garbage text`) but **do not** press Enter. Now `Alt` → Output History → **Rerun Focused Input**. **(3)** Press Enter. **(4)** `Ctrl+Shift+Up` to a different cell, invoke Rerun Focused Input, press Enter. **(5)** `Ctrl+Shift+D`. | **(1)** **"No focused cell. Navigate to a command cell first."** — nothing inserted, nothing run. **(2)** Announce **"Command from cell N inserted at the prompt. Press Enter to run."**; `echo hello` is inserted at the cursor and the shell has **not** run anything yet (no auto-run). **Known gap:** the prior `garbage text` is currently **NOT** cleared (it remains on the line — the tracked Esc-clear bug); inserting-without-clearing here is the *expected current* behaviour until that deferred fix lands, not a P3 fail. **(3)** Clear the line manually if needed, then *your* Enter runs the command; a fresh cell with new output is produced and auto-narrated. **(4)** The prompt line is replaced with that other cell's command, again not auto-run until your Enter. The menu item is under Output History, keyboard-reachable, announces **no** shortcut. **Bundle checks:** `ADR 0007 Phase 3 rerun-input inserted. SourceCellSeq={…} CmdLen={…}` — counts only, **no command text** in the log (it appears on the prompt line for you, never in the bundle). **Fail signals (genuine):** a single invocation *auto-running* the command (must require your Enter); the command **not** inserted at the cursor at all; the item missing or announcing a phantom shortcut; command text in the `Ctrl+Shift+D` bundle. **NOT a fail (known/deferred):** the line-clear not happening — that is the tracked Esc-clear gap. |

| **52-ADR7-P6a** | **✅ FEATURE PASSED 2026-05-17 — D8 CONTROL TYPE RATIFIED.** Maintainer NVDA dogfood (installed preview, post #408 / `c49f8e5`): the flat `ListBox` → UIA `List` **reads cleanly as a standard list**; `Ctrl+Shift+Left/Right` pane switch works both directions, SpeechCursor `Ctrl+Shift+Up/Down/End` independent, startup focus on the terminal, **no double-speak** (the parallel focus-marker behaved as designed) — speaks each item with its "N of M" position. **D8 control type RATIFIED** (the `List` itself reads cleanly); `Tree` remains the deferred Phase-4/5 growth path. **Surfaced (NOT a D8-blocker — pane-switch focus plumbing, not the `List`):** after the WPF `Menu` (its own focus scope) was entered during back-and-forth switching, focus got trapped in the menu — `element.Focus()` didn't pull the keyboard-focus device out of the Menu scope, so arrows kept driving menu items, unrecoverable even after switching edit→list. Plus a low-vision request: make the selected row visibly obvious (the list IS drawn — real Grid column — but the default selection visual is faint). **→ 6a-2b follow-up** (`Keyboard.Focus` cross-scope in both pane handlers + select-latest-on-enter + high-contrast `SystemColors` selection recolor + larger font); **re-dogfood `52-ADR7-P6a-fix`** below. **Phase-6+ proceed is gated on that re-dogfood** (D8 itself stays ratified). Original procedure retained below for re-runs. — **ADR 0007 Phase 6a-2b — the focusable cell-history list + `Ctrl+Shift+Left/Right` pane switch. THIS DOGFOOD IS THE HARD D8 CONTROL-TYPE RATIFICATION GATE — it ratifies the flat `ListBox` → UIA `List` (or triggers the Tree/other fallback) and gates everything after Phase 6.** Installed preview (post the 6a-2b PR), NOT a dev run; restart NVDA if the UIA-notification wedge bites. **(0) Startup focus regression:** launch; confirm focus lands on the **terminal** as before (NVDA: the terminal document, not the list — the load-bearing `Loaded → TerminalSurface.Focus()` must be unchanged). **(1) List populates:** under **cmd** run `echo one` ⏎, `dir` ⏎, `echo two` ⏎. **(2) Pane switch in:** press **`Ctrl+Shift+Left`**. **(3) Native list nav:** with the list focused, press `Down`/`Up`/`Home`/`End` through the items; try first-letter type-ahead (`c` / `o`). **(4) Pane switch out:** press **`Ctrl+Shift+Right`**. **(5) Switch from inside the list:** `Ctrl+Shift+Left` again, then `Ctrl+Shift+Right` (must work while the list — not the terminal — has focus). **(6) SpeechCursor independence (regression):** `Ctrl+Shift+Up`/`Down`/`End` still navigate the manual review path exactly as `52-ADR7-P1` (separate code; must be unaffected). **(7) Menu:** `Alt` → Interface → **Pane** → confirm **Focus History Pane** / **Focus Terminal Pane** present, keyboard-reachable, announcing their `Ctrl+Shift+Left` / `Ctrl+Shift+Right` shortcut. **(8)** `Ctrl+Shift+2` → PowerShell, repeat (1)–(4) briefly. **(9)** `Ctrl+Shift+D`. | **(0)** Startup focus = terminal, unchanged. **(2)** Focus moves to the history pane; **NVDA announces it natively** — "Cell history, list" (UIA `List`) or equivalent; **we do NOT manually announce** (the cue, if any, must not re-read the element). **(3) THE RATIFICATION:** NVDA browses it as a **standard list** — each arrow step reads **exactly once** "`Command: echo one`" / "`Output: …`" / etc. with **"N of M"** position, Home/End jump, type-ahead works. **No double-speak** (a doubled/echoed item read = conformance-#1 breach: we manually announced — must be fixed). The list shows, in order, one row per appended item ("Command: …" then "Output: …"; whitespace-only sides skipped). **(4)/(5)** Focus returns to the terminal (NVDA announces the terminal document); the gesture works from **either** pane (Window InputBinding, not the terminal-only filter). **(6)** SpeechCursor `Ctrl+Shift+Up/Down/End` behave **identically to `52-ADR7-P1`** — the list and the manual review path are independent (ADR 0007 6a-2b conformance #2/#3). **(7)** Both menu items present, reachable, shortcut announced; invoking = the gesture. **Bundle checks:** `ADR 0007 Phase 6a-2b list append. CellSeq=… Kind=… Count=…` (one per appended item, monotonic `Count`); `… list focus-marker. Index=… CellSeq=… Kind=…` (one per arrow-landing in the list — proves the **universal bus was fed in parallel** to NVDA's native read); `… pane switch. Target=history|terminal` (one per gesture). No cell text in the bundle (logging discipline). **D8 outcome:** if (3) reads cleanly as a standard list → **flat `List` RATIFIED** for the flat cut (Tree deferred to Phase 4/5, typed model unchanged); if NVDA mis-handles it (no list semantics, broken item nav, review-cursor conflict) → **do NOT proceed past Phase 6**; capture the bundle + exact NVDA output and fall back per D8 (Tree, or escalate). **Fail signals:** pane gesture swallowed / no focus move (keyboard-filter or InputBinding regressed); double-speak on item nav (conformance #1 breach); list empty after commands (Appended subscription / Dispatcher marshalling broke); SpeechCursor nav changed (entanglement — conformance #2 breach); startup focus not on the terminal (`Loaded` regressed); a 6a-2b bundle line missing for an action that occurred; command/output text in the bundle. |

| **52-ADR7-P6a-fix** | **✅ PASSED 2026-05-17 — superseded & expanded by the consolidated `52-ADR7-P6b` round (#410–#415); see that row for the as-shipped scope + verdict. Original procedure retained below for re-runs.** — 6a-2b follow-up — pane-switch FocusScope fix + low-vision visible selection (re-dogfood of the `52-ADR7-P6a` surfaced issues; D8 already ratified). Installed preview (post the follow-up PR), NOT a dev run. **(1)** under **cmd** run `echo one` ⏎, `dir` ⏎, `echo two` ⏎. **(2)** **`Ctrl+Shift+Left`** → confirm focus enters the list AND a row is already selected/announced (the latest), not an empty list. **(3) The trap repro:** press `Alt` → Interface → **Pane** → arrow within the menu a little → `Esc`. Now press **`Ctrl+Shift+Left`** → you MUST land back in the history list with **arrows driving the list items, not the menu**. Repeat the menu-open-then-pane-switch a few times + back-and-forth `Ctrl+Shift+Left`/`Right` rapidly. **(4)** Low-vision visual: with the list focused, eyeball the screen (or have a sighted check / screenshot) — the selected row is **clearly highlighted** (strong blue/white, bold-ish, larger font) and the highlight **stays visible after `Ctrl+Shift+Right`** moves focus away. **(5)** Regression: `Ctrl+Shift+Up/Down/End` SpeechCursor unchanged; startup focus = terminal; no double-speak on list nav. **(6)** `Ctrl+Shift+D`. | **(2)** Entering the pane selects the latest row → NVDA announces it immediately (e.g. "Output: …, 4 of 4"); not a focus-with-no-item dead end. **(3) THE FIX:** after menu interaction, the pane switch reliably returns keyboard focus to the list — arrows move through **history items**, never menu items; never gets stuck in the menu regardless of how much back-and-forth. **(4)** The selected row is visibly high-contrast and readable on screen, and the selection highlight persists when focus leaves (low-vision can see where the NVDA cursor is). **(5)** SpeechCursor manual nav, startup focus, and single-utterance list reads all unchanged from `52-ADR7-P6a`. **Bundle:** `ADR 0007 Phase 6a-2b pane switch. Target=history|terminal` per gesture; `… list focus-marker …` on the auto-selected row at entry. **Fail signals:** focus still trappable in the menu after menu use (the `Keyboard.Focus` cross-scope fix didn't take); entering the pane lands on no item / silent; selection not visibly distinct or invisible once focus leaves (styling ineffective); any `52-ADR7-P6a` regression (double-speak, SpeechCursor changed, startup focus moved). |
| **52-ADR7-P6b** | **✅ PASSED 2026-05-17 — consolidated cycle row (supersedes `52-ADR7-P6a-fix`).** Maintainer dogfood across the #410–#415 sequence (installed previews, iterative): "everything seems good." Verified: (a) pane keys spatially correct — `Ctrl+Shift+Left`=terminal, `Ctrl+Shift+Right`=history — with a distinct per-pane earcon; (b) the menu focus-trap no longer recurs across many round-trips (dispatcher-deferred focus onto the selected `ListBoxItem`); NVDA's focus rectangle wraps the focused row; (c) the selected row is strongly highlighted and the highlight persists when focus leaves the pane (own `ListBoxItem` ControlTemplate, theme-independent); (d) per-cell ops (`Ctrl+Shift+C` copy / copy-command / copy-output / rerun) act on the **list's selected cell** (by `CellId`), "no cell selected" on a notice row; (e) empty list holds focus via the placeholder; (f) bold per-panel titles; light-scheme inversion as requested; (g) shell hot-switch appends a "Shell switched to <name>." marker row, history not reset; (h) menu reorg ×2 (Interface · Terminal · Cell History · Diagnostics · About; Prompt Path→Terminal; Open Data Folder→Diagnostics; Window→Interface; Session; enabled `(not yet implemented)` op-set); (i) About menu browser links open the right pages + the Version item reads the build version + git short SHA. **Known/deferred (NOT fails, recorded):** the input-test seals **no command cell** (ADR 0004 drop-on-None / boundary detection — the *next* track, see SESSION-HANDOFF § Next stage); no live output trickle (ADR 0007 Phase 4, by design); focused-item outline-on-blur deferred to the stable Tree. — **ADR 0007 Phase 6b consolidated round (#410–#415).** Re-run procedure: combine the `52-ADR7-P6a` and `52-ADR7-P6a-fix` procedures above, plus: (j) open `Alt` → Cell History → Navigation and confirm the four "Move to …" items are **announced by NVDA** (enabled placeholders, not silent/disabled); (k) `Alt` → About → Version → the version line reads version + `+<sha>` (matches `Ctrl+Shift+H`); (l) About → Open GitHub Repo / Submit a Feature Request / Submit an Issue each open the correct page in the default browser; (m) `Ctrl+Shift+1/2/3` shell switch leaves prior history rows in place and adds the marker row. **Fail signals:** any `52-ADR7-P6a`/`-fix` fail signal; Navigation items silent under NVDA (placeholders left disabled); per-cell op acting on the SpeechCursor cell instead of the list selection; menu top-level order wrong; an About link opening the wrong page; Version item static/empty (runtime header not set). |
