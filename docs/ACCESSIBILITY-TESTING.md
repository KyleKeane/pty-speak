...
| **47-24** | **Mid-evaluation input earcon**: with earcons enabled (View → Earcons → Enabled), run `set /p var=Enter your text:` at the cmd prompt. | NVDA announces "Enter your text:" (idle-flush), AND a brief 3000 Hz click plays at the same time — the same earcon the tuple-final `ReadyForInput` path uses. Pre-fix: only the announce; no audible "you can type now" cue. Confirms by listening; visually, grep the bundle FileLogger for `idle-flush` source attribution on a `ReadyForInput` OutputEvent. Mute toggle (View → Earcons → Muted) silences it alongside the tuple-final cue. |
