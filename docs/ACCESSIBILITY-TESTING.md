...
| **52-ADR7-P3** | **Mechanics PASSED 2026-05-17; behaviour simplified per maintainer UX direction — re-dogfood the NEW behaviour.** The two-step arm/confirm was dropped (it was the conservative reading of the ADR "confirm gesture" open decision; the maintainer resolved it in favour of the simple flow). **New behaviour:** Rerun Focused Input now **clears the current prompt line and inserts the focused cell's command at the prompt — it does NOT auto-run**; the user reviews the line and presses Enter themselves (their explicit Enter is the safety affordance). Same `insertAtPromptClearingLine` (Esc-clear, no `\r`) path the diagnostic test-script insertion uses — the two are now one shared primitive. — **ADR 0007 Phase 3 — rerun focused input** (behavioural; **menu-only**, no accelerator). Under **cmd**: **(1)** Fresh session, nothing navigated: `Alt` → Output History → **Rerun Focused Input**. **(2)** Run `echo hello` ⏎ (cell auto-followed). Type some junk at the prompt (e.g. `garbage text`) but **do not** press Enter. Now `Alt` → Output History → **Rerun Focused Input**. **(3)** Press Enter. **(4)** `Ctrl+Shift+Up` to a different cell, invoke Rerun Focused Input, press Enter. **(5)** `Ctrl+Shift+D`. | **(1)** **"No focused cell. Navigate to a command cell first."** — nothing inserted, nothing run. **(2)** Announce **"Command from cell N inserted at the prompt. Press Enter to run."**; the prompt line now reads exactly `echo hello` — the `garbage text` you typed was **cleared/replaced**, and the shell has **not** run anything yet (no auto-run). **(3)** *Your* Enter runs `echo hello`; a fresh cell with new `hello` output is produced and auto-narrated. **(4)** The prompt line is replaced with that other cell's command, again not auto-run until your Enter. The menu item is under Output History, keyboard-reachable, announces **no** shortcut. **Bundle checks:** `ADR 0007 Phase 3 rerun-input inserted. SourceCellSeq={…} CmdLen={…}` — counts only, **no command text** in the log (it appears on the prompt line for you, never in the bundle). **Fail signals:** a single invocation *auto-running* the command (must require your Enter); the prior `garbage text` **not** cleared (line appended instead of replaced); the item missing or announcing a phantom shortcut; command text in the `Ctrl+Shift+D` bundle. |
