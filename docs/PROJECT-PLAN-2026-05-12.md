...
> **Snapshot**: 2026-05-12 (strategic-track plan; **superseded
> for current/active state by
> [`CYCLE-52-R5-PLAYBOOK.md`](CYCLE-52-R5-PLAYBOOK.md)** — Cycle
> 52 R1–R4+R4c shipped 2026-05-16, see the change log below +
> the playbook for what's next). This file remains the dated
> strategic-track record per the Track E E5 discipline; it is
> not the start-here doc.
> **Status**: strategic-track plan. Succeeds
...
| 2026-05-16 | Cycle 52 R4c + FREEZE + R5 scoping (this entry) | **R4c** (#369–#372, post-#357) — cmd emits a boundary-only deferred `;D` (`CommandFinished None`, no exit code; cmd OS-level limit) completing its OSC-133 stream so R5/R6 are shell-agnostic in the core; consumer-side augmentation + stray-`;D` gate; folds into the one R1–R4+R4c foundation dogfood (matrix adds `52-R4c`). Deferrals captured (#370–#371): `stripNextPrompt`→both-edges positional region-cut, SpeechCursor→IOCell-history nav + per-cell ops, review-cursor-document open decision, NVDA current-line propagation, multi-interrupt IOCell structure, fast-type/recall command-echo race — all in ADR 0006 §"Deferred to R6+". **Standing decision (#372): cmd announce-heuristic FREEZE** — the post-`5518f5c` bundle proved the IOCell substrate sound; every remaining cmd defect is in the racy announce-reconstruction layer; do NOT patch cmd announce heuristics before R5; cmd has no banner (architectural, not a bug). R5 (PowerShell) + the P1–P5 pre-R5 old-code pruning sequence meticulously scoped in **[`CYCLE-52-R5-PLAYBOOK.md`](CYCLE-52-R5-PLAYBOOK.md)** (the new start-here). CYCLE-46/49/51 plans archived → `archive/cycle-closed/`. |
