...
| `baseline/cycle-27-multistate-paradigm` | <pre># Resolve the merge SHA of Cycle 27 (this PR) once it lands<br># on `main`, then run the tag block below.<br># `git log --oneline origin/main` and pick the squash-merge<br># commit titled `feat(hotkeys,views): Cycle 27 ...`.<br>git fetch origin main<br>git tag -a baseline/cycle-27-multistate-paradigm \\<br>  &lt;27-merge-sha&gt; \\<br>  -m "Cycle 27 ship: multi-state menu paradigm canon end-to-end — parallel MultiStateCommand DU + MultiStateDef/MultiStateOption records + multiStateBuiltIns/multiStateAllCommands/multiStateOf/multiStateNameOf registry surfaces in HotkeyRegistry.fs; new bindMultiState helper + MultiStateBinding capture in Program.fs mirrors bindHotkey shape minus KeyBinding step (multi-state is menu-only by canon); migrates MuteEarcons (Ctrl+Shift+M) → EarconsMode (View → Earcons → Enabled / Muted) and ToggleDebugLog (Ctrl+Shift+G) → LoggingLevel (View → Logging Level → Information / Debug); both keyboard accelerators dropped per maintainer's hotkey-count working-memory ceiling; new MultiStateRegistryTests.fs with 11 fixtures pins shape contracts; AppReservedHotkeysMirrorTests parity preserved automatically (both surfaces shed the same two entries); active option indicated via WPF MenuItem.IsCheckable=true + IsChecked refreshed on SubmenuOpened; awaiting maintainer NVDA-matrix walkthrough on next preview cut; anchor at the Cycle 27 merge commit"<br>git push origin baseline/cycle-27-multistate-paradigm</pre> |
