...
| `baseline/cycle-24-sessionmodel-persistence` | [#203](https://github.com/KyleKeane/pty-speak/pull/203), [#206](https://github.com/KyleKeane/pty-speak/pull/206), [#208](https://github.com/KyleKeane/pty-speak/pull/208), [#209](https://github.com/KyleKeane/pty-speak/pull/209), [#210](https://github.com/KyleKeane/pty-speak/pull/210) + this PR | _(post-Stage-7 substrate cycle; NVDA matrix landed in 24e)_ | Cycle 24 ship: SessionModel persistence substrate end-to-end. **24a (#203):** TOML config schema (`[session_persistence] mode = "memory_only" \| "session_log" \| "always"`, `output_dir`, `format = "jsonl"`, `max_session_size_mb`); `Config.SessionPersistence` reader; default-mode `memory_only` keeps the substrate compatible with pre-Cycle-24 users. **24b (#206):** pure JSONL serializer `SessionModel.formatTupleAsJsonl` (decades-stable `JsonlSchemaVersion = 1`; UTF-8 no-BOM; deterministic field ordering; lone-surrogate throws at emit time; angle-bracket escape superset for safer log viewers). **24c (#208):** bounded-channel async file writer `SessionLogWriterSink` (256-cap channel, single-reader drain task, `FileShare.ReadWrite` for concurrent inspection, `Append`+`Flush` per batch, `%LOCALAPPDATA%\PtySpeak\sessions\session-<id>.jsonl` per shell session). **24d-1 (#209):** `EnqueueSync` for `Always` mode — per-tuple `TaskCompletionSource` flows through the channel, drain signals after batch flush; 10s timeout with graceful degradation; mixed `Enqueue`+`EnqueueSync` calls preserve FIFO order on disk. **24d-2 (#210):** env-var VALUE-based sanitisation — captures values for deny-listed env vars at startup (Stage 7 pattern: `*_TOKEN`/`*_SECRET`/`*_KEY`/`*_PASSWORD`; `ANTHROPIC_API_KEY` exempted; min-length 16 chars), redacts occurrences in `commandText`/`outputText`/`promptText`/`extraParams` values with `<REDACTED:UPPERCASE_NAME>` markers; substring-overlap-safe (longer values match first); substrate's in-memory History stays honest, only the persistence layer redacts. **24e (this PR):** `Ctrl+Shift+S` diagnostic hotkey announces the active session-log file path (verbose format: `Session log mode <mode>; path <full-path>.` for SessionLog/Always; `Session log mode memory_only; no file.` for MemoryOnly) + 6-row NVDA validation matrix in `docs/ACCESSIBILITY-TESTING.md` Cycle 24 subsection. Spec authority: SESSION-MODEL.md "Persistence modes" + "Env-var-value sanitisation (Cycle 24d-2)". NVDA-validated 2026-05-09 (manual smoke). Anchor commit is the 24e merge SHA. |
...
| `baseline/cycle-24-sessionmodel-persistence` | <pre># Resolve the merge SHA of the 24e closure PR (this PR)<br># once it lands on `main`, then run the tag block below.<br># `git log --oneline origin/main` and pick the squash-merge<br># commit titled `feat(terminal-app): Cycle 24e closure ...`.<br>git fetch origin main<br>git tag -a baseline/cycle-24-sessionmodel-persistence \\<br>  &lt;24e-merge-sha&gt; \\<br>  -m "Cycle 24 ship: SessionModel persistence substrate end-to-end — TOML schema (24a) + pure JSONL serializer (24b) + bounded-channel async writer (24c) + Always-mode synchronous flush (24d-1) + env-var VALUE sanitisation (24d-2) + Ctrl+Shift+S diagnostic hotkey + NVDA matrix (24e); 5 sequenced PRs #203 #206 #208 #209 #210 plus the 24e closure PR; NVDA-validated 2026-05-09; anchor at the 24e merge commit as the most recent Cycle 24 substrate constituent"<br>git push origin baseline/cycle-24-sessionmodel-persistence</pre> |
| `baseline/cycle-26-app-menu` | <pre># Resolve the merge SHA of Cycle 26d (this PR) once it lands<br># on `main`, then run the tag block below.<br># `git log --oneline origin/main` and pick the squash-merge<br># commit titled `docs: Cycle 26d ...`.<br>git fetch origin main<br>git tag -a baseline/cycle-26-app-menu \\<br>  &lt;26d-merge-sha&gt; \\<br>  -m "Cycle 26 ship: app-menu mini-cycle end-to-end — menu skeleton + UIA plumbing (26a, PR #225) + populated 5-menu structure with 14 named MenuItems wired to shared RoutedCommand instances + Hotkey.Key/Modifiers option-typing + AppReservedHotkeysMirrorTests F#/C# parity invariant (26b, PR #226) + first menu-only AppCommand RunProcessCleanupScript surfaced via Diagnostics → Test Process Cleanup (26c, PR #227) + NVDA matrix rows + How-to-add-a-menu-item recipe in USER-SETTINGS.md (26d, this PR); awaiting maintainer NVDA-matrix walkthrough on next preview cut; anchor at the 26d merge commit as the most recent Cycle 26 constituent"<br>git push origin baseline/cycle-26-app-menu</pre> |
| `baseline/cycle-27-multistate-paradigm` | <pre># Cycle 27 merged as commit 1262dbb (PR #229).<br># Maintainer NVDA-validated 2026-05-09 ("multi-state menu items work exactly as expected").<br>git fetch origin main<br>git tag -a baseline/cycle-27-multistate-paradigm \\<br>  1262dbb407de0579b57c3f48cc6cc55bde2e8a05 \\<br>  -m "Cycle 27 ship: multi-state menu paradigm canon end-to-end — parallel MultiStateCommand DU + MultiStateDef/MultiStateOption records + multiStateBuiltIns/multiStateAllCommands/multiStateOf/multiStateNameOf registry surfaces in HotkeyRegistry.fs; new bindMultiState helper + MultiStateBinding capture in Program.fs mirrors bindHotkey shape minus KeyBinding step (multi-state is menu-only by canon); migrates MuteEarcons (Ctrl+Shift+M) → EarconsMode (View → Earcons → Enabled / Muted) and ToggleDebugLog (Ctrl+Shift+G) → LoggingLevel (View → Logging Level → Information / Debug); both keyboard accelerators dropped per maintainer's hotkey-count working-memory ceiling; new MultiStateRegistryTests.fs with 11 fixtures pins shape contracts; AppReservedHotkeysMirrorTests parity preserved automatically (both surfaces shed the same two entries); active option indicated via WPF MenuItem.IsCheckable=true + IsChecked refreshed on SubmenuOpened; maintainer NVDA-validated 2026-05-09; anchor at the Cycle 27 merge commit"<br>git push origin baseline/cycle-27-multistate-paradigm</pre> |
| `baseline/cycle-28-window-menu-and-cleanup` | <pre># Resolve the merge SHA of Cycle 28 (this PR) once it lands<br># on `main`, then run the tag block below.<br># `git log --oneline origin/main` and pick the squash-merge<br># commit titled `feat(views,hotkeys): Cycle 28 ...` or similar.<br>git fetch origin main<br>git tag -a baseline/cycle-28-window-menu-and-cleanup \\<br>  &lt;28-merge-sha&gt; \\<br>  -m "Cycle 28 ship: foundation cleanup + Window menu — adds top-level Window menu (Close Window + Exit) as new menu-only AppCommands CloseWindow + ExitApp; fixes stale Ctrl+Shift+L comment in Program.fs:685 (replaced with live Ctrl+Shift+P → OpenDataFolder; old hotkey was removed in Cycle 25b-1a); refreshes PROJECT-PLAN-2026-05-09.md in place with Status update post Cycle 27 closure + Sequencing block updates; informational hotkey-menu-coverage audit confirmed all 12 remaining gesture-bearing AppCommands ARE represented in the menu; awaiting maintainer NVDA walkthrough of new Window-menu rows on next preview cut; anchor at the Cycle 28 merge commit"<br>git push origin baseline/cycle-28-window-menu-and-cleanup</pre> |
