...
| **In-flight branch** | `claude/cycle-29-selectionprofile-substrate` — **Cycle 29a SelectionDetector substrate** (this PR). First of three sequenced PRs (29a/29b/29c) closing spec Stage 8e for `[output-selection]` (Claude tool-use prompt reads as flat text). Ships pure functional `SelectionDetector.fs` (tech-plan §8.1 heuristic) + `SelectionExtensions` schema constants module in `OutputEventTypes.fs` + 19-fact `SelectionDetectorTests.fs`. The detector compiles standalone, exports a clean public API, and is fully unit-tested — but is NOT yet wired into `OutputDispatcher` (NVDA behaviour unchanged). Wiring lands in Cycle 29b alongside `SelectionProfile.fs`. Most recent merge before this branch: PR #230 (Cycle 28, commit `5329fa3`). |
| **Next stage** | **Cycle 29b — SelectionProfile + Program.fs wiring**. Add `src/Terminal.Core/SelectionProfile.fs` as cousin of `EarconProfile.fs` (pattern-match on SelectionShown / SelectionItem / SelectionDismissed → emit NVDA + FileLogger ChannelDecisions). Wire `SelectionDetector` into the composition root in `Program.fs` (~lines 932-944, 1148, 1305-1322; keystroke handler search `KeyEncoding.encode` call sites). Add `SelectionProfileTests.fs` (~6 facts) + extend `OutputDispatcherTests.fs` with one fact for the active profile set. NVDA starts speaking selection events as text. Then **Cycle 29c — TOML loader**: add `[profile.selection]` section to `Config.fs`; add `SelectionConfigTests.fs` (~4 facts); composition-root re-wiring through TOML. Closes spec Stage 8e-A scope. **Cycle 8e-B + 8e-C** continue the sub-cycle (UIA listbox peer + arrow-key round-trip; separate plan-mode passes). **Open follow-ups (cross-cutting; not blocking)**: (a) Screen-buffer runtime resize (Phase 2). (b) Stable-baseline tag pushes including `baseline/stage-7-claude-roundtrip`, the Cycle-24e candidate, `baseline/cycle-26-app-menu`, `baseline/cycle-27-multistate-paradigm`, `baseline/cycle-28-window-menu-and-cleanup`, and the new `baseline/cycle-29a-selectiondetector-substrate` per [`docs/CHECKPOINTS.md`](CHECKPOINTS.md) — local sandbox can't push tags, awaiting maintainer sweep. (c) MAY-4.md Concern 3 (navigable streaming response queue) — naturally subsumed by SessionModel + ReplPathway (Phase 2). (d) Manual NVDA matrix walkthrough of the Cycle 28 Window-menu rows in `docs/ACCESSIBILITY-TESTING.md` against the next preview cut. **Cycle 27 NVDA matrix is validated** as of 2026-05-09 ("multi-state menu items work exactly as expected"). |
