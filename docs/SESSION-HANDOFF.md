...
| **In-flight branch** | `claude/cycle-26d-docs-and-nvda-matrix` — **Cycle 26d NVDA matrix + docs refresh** (this PR; doc-only). Adds the Cycle 26 "App menu" section to `docs/ACCESSIBILITY-TESTING.md` (5 NVDA matrix rows + diagnostic decoder + ADR-style note about the parallel-surface decision); adds the "How to add a menu item" recipe to `docs/USER-SETTINGS.md`; updates this file's "Where we left off" through Cycle 26; updates `CLAUDE.md` line 462 to past tense (cite Cycle 26c instead of "future cycle's app menu will surface this script"); adds `baseline/cycle-26-app-menu` to `docs/CHECKPOINTS.md` pending-tag-pushes table. Most recent merge before this branch: PR #227 (Cycle 26c, commit `65119665`). |
| **Next stage** | **Decide next cycle direction.** Cycle 26 closes; menu surface is shipped + populated + extensibility-recipe-documented. Two strong candidates on deck per [`docs/PROJECT-PLAN-2026-05-09.md`](PROJECT-PLAN-2026-05-09.md): **(a) Output framework cycle (Part 3)** — subsumes `spec/tech-plan.md` Stages 8 + 9; designed in `spec/event-and-output-framework.md`; multi-cycle work; plan-mode per sub-stage. The strategic priority. **(b) Future menus and per-output settings** — the maintainer flagged Window menu, Display (rename of View?), Preferences/Settings menu (with first user-toggle: hide CWD on cmd) as anticipated additions. The Cycle 26 extension recipe in `USER-SETTINGS.md` documents the three-edit pattern; landing one of these as a smaller cycle would exercise the recipe end-to-end before the framework cycle's heavier cadence. **Parked**: Cycle 25b-2 (FlaUI/UIA E2E runner) — design sketch retained in this file's "Cycle 25 in-flight handoff" section; trigger to revisit is autonomous-validation discipline wanted OR a regression slips through manual NVDA validation. **Open follow-ups (cross-cutting; not blocking the next cycle)**: (a) Screen-buffer runtime resize (Phase 2). (b) Stable-baseline tag pushes including `baseline/stage-7-claude-roundtrip`, the Cycle-24e candidate, and the new `baseline/cycle-26-app-menu` per [`docs/CHECKPOINTS.md`](CHECKPOINTS.md) — local sandbox can't push tags, awaiting maintainer sweep. (c) MAY-4.md Concern 3 (navigable streaming response queue) — naturally subsumed by SessionModel + ReplPathway (Phase 2). (d) Manual NVDA matrix walkthrough of the new Cycle 26 rows in `docs/ACCESSIBILITY-TESTING.md` against the next preview cut. |
