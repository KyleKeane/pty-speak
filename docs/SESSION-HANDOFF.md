...
| **In-flight branch** | `claude/event-and-output-framework-spec` — the substrate spec PR for the post-Stage-7 framework cycles. Adds [`spec/event-and-output-framework.md`](../spec/event-and-output-framework.md), supersedes `spec/tech-plan.md` §8 / §9 in-place with one-paragraph headers (original content preserved), reframes §10 as the first non-built-in framework consumer, and cross-references `docs/PROJECT-PLAN-2026-05.md` Part 3 + Part 4 to point at the new spec. Doc-only PR; CI is no-op. Earlier substrate-cleanup bundle: PR-N #146, PR-O #147, PR-P #148 + research seed cross-references PR #149 + README/docs reorg PR #150 all merged through 2026-05-04. |
| **Next stage** | **Sub-stage 8a — OutputEvent + Channel + NVDA-channel retrofit** per [`spec/event-and-output-framework.md`](../spec/event-and-output-framework.md) Part C. The new spec consolidates Part 3 + Part 4's substrate concerns into v1 commitments: RawInput envelope + Intent layer + Dispatcher (input side); OutputEvent + Profile + Channel (output side). The retrofit follows walking-skeleton discipline — eleven sub-stages (8a-8f for output cycle, 9a-9d for input cycle, plus the §10 reframe) each ship their own multi-PR mini-cycle with NVDA validation. Stage 8a re-routes the existing pipeline behaviour-identically through the new substrate; Stage 8b promotes the Coalescer to the Stream profile (per the PR-N docstring contract); Stage 8c-8f add the FileLogger / Earcons / Selection / per-shell mapping channels and profiles; Stage 9a-9d add the input-side substrate; Stage 10 (review mode + quick-nav) becomes the first non-built-in framework consumer. Open follow-ups: (a) Screen-buffer runtime resize (Phase 2 stage). (b) Stable-baseline tag pushes including `baseline/stage-7-claude-roundtrip` per [`docs/CHECKPOINTS.md`](CHECKPOINTS.md). (c) MAY-4.md Concern 3 (navigable streaming response queue) — its own spec when Stage 10 starts. |
