...
| | |
|---|---|
| **Last merged stages** | **Stages 0 → 7 + 11** all merged to `main`, plus the retroactively-formalized Stages 4a / 4b / 5a. **Stage 7 shipped 2026-05-03 across 11 sequenced PRs (A → K) plus the doc-purpose wrap-up PR-L:** PR-A #131 (env-scrub PO-5), PR-B #132 (shell registry + `PTYSPEAK_SHELL`), PR-C #134 (hot-switch hotkeys `Ctrl+Shift+1/2`), PR-D #135 (NVDA validation matrix + STAGE-7-ISSUES seeding), PR-E (`Ctrl+Shift+G` debug toggle), PR-F (`Ctrl+Shift+H` health check + `Ctrl+Shift+B` incident marker + 5s heartbeat), PR-G (`Ctrl+Shift+;` dispatcher-deadlock fix), PR-H (500-char announce cap stopgap; tracked by #139 for Stream-profile removal), PR-I #141 (silent reader-loop shutdown on shell-switch + `currentShell` tracking), PR-J #142 (PowerShell as third built-in shell + reordered hotkeys to `+1`=cmd / `+2`=PowerShell / `+3`=Claude + Ctrl+Shift+H liveness probe + Ctrl+Shift+D inline shell-process snapshot), PR-K #143 (env-scrub allow-list expanded with Windows runtime baseline after PowerShell + claude.exe both died on spawn during 2026-05-03 NVDA pass; "stripped 0" log line replaced with kept/parent/denied counts). PR-L (this PR) ships the doc-purpose audit + Stage 7 wrap-up. **Stage 7 NVDA validation: green** (maintainer confirmed cmd / PowerShell / Claude all spawn, stay alive, and announce correctly post-PR-K). |
| **Last shipped release** | `v0.0.1-preview.43` (latest code-bearing preview noted in plan; cadence may have advanced through subsequent previews. Maintainer cuts the next preview from `main` after PR-L lands.) |
| **In-flight branch** | _None._ Stage 7 + the doc-purpose wrap-up are complete. Open small follow-up: GitHub Issue #117 (coalescer cross-row spinner detection redesign + per-key static-row fix) is queued as PR-M but separately mergeable — its CI gate is the new CoalescerTests fixtures, and a manual NVDA "type fast in cmd, watch for spinner false-positives" pass is the validation. |
| **Next stage** | **Output framework cycle (Part 3)** per [`docs/PROJECT-PLAN-2026-05.md`](PROJECT-PLAN-2026-05.md). Begins with research phase (`docs/research/OUTPUT-FRAMEWORK-PRIOR-ART.md` — to be written) reading [`docs/STAGE-7-ISSUES.md`](STAGE-7-ISSUES.md) as design input. The headline finding to architect around: verbose readback (Stage 5's whole-screen re-announcement on every event; PR-H's 500-char cap is the stopgap; #139 + #115 + #117 are the relevant tickets). After the research phase, an RFC, then eight sub-stages each with NVDA validation. Then the **Input framework cycle (Part 4)** with the same shape, then **Stage 10** (review-mode + quick-nav, the first non-built-in consumer of the framework taxonomy). Open follow-ups logged for future stages: (a) Screen-buffer runtime resize — Stage 6 resizes the PTY but the in-process `Cell[,]` Screen grid stays at construction-time 30×120 (Phase 2 stage). (b) Stable-baseline tag pushes including the new `baseline/stage-7-claude-roundtrip` row added to [`docs/CHECKPOINTS.md`](CHECKPOINTS.md) by PR-L. |
