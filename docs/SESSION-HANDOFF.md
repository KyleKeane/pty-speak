...
| **In-flight branch** | `claude/cycle-29b-selectionprofile-wiring` — **Cycle 29b SelectionProfile + Program.fs wiring** (this PR). Second of three sequenced PRs (29a/29b/29c) closing spec Stage 8e. NVDA now reads Claude's selection prompts as text. New `SelectionProfile.fs` (cousin of `EarconProfile.fs`); detector adjusted to empty-Payload + ItemIndex schema; `Program.fs` composition wiring (registration + mutable detector + `runDetector` extension + 3 reset hooks); 13-fact `SelectionProfileTests.fs` + `ItemIndex` schema fact added to `SelectionDetectorTests.fs`. **Keystroke wiring deferred to 8e-C** per maintainer's plan-mode choice. Most recent merge before this branch: PR #231 (Cycle 29a, commit `47c6905`). |
| **Next stage** | **Cycle 29c — TOML loader**: add `[profile.selection]` section to `Config.fs` so the maintainer can tune `HighlightDetectionThresholdMs` / `DismissalGraceMs` / `KeystrokeCorrelationWindowMs` / `MinConfidence` via `Ctrl+Shift+E`. Composition-root re-wiring (replace `SelectionDetector.defaultParameters` with the loaded values). Add `SelectionConfigTests.fs` (~4 facts). Closes spec Stage 8e-A scope. **Cycle 8e-B + 8e-C** continue the sub-cycle: UIA listbox peer (`TerminalListAutomationPeer` + `TerminalListItemAutomationPeer`; replaces text-only RenderText with RenderRaw UIA listbox semantics; spec tech-plan §8.2-§8.5) + arrow-key round-trip (`ISelectionItemProvider.Select` → PTY arrow byte sequence + the deferred keystroke-feedback wiring). Each gets its own plan-mode pass. **Open follow-ups (cross-cutting; not blocking)**: (a) Screen-buffer runtime resize (Phase 2). (b) Stable-baseline tag pushes including `baseline/stage-7-claude-roundtrip`, the Cycle-24e candidate, `baseline/cycle-26-app-menu`, `baseline/cycle-27-multistate-paradigm`, `baseline/cycle-28-window-menu-and-cleanup`, `baseline/cycle-29a-selectiondetector-substrate`, and the new `baseline/cycle-29b-selectionprofile-wiring` per [`docs/CHECKPOINTS.md`](CHECKPOINTS.md) — local sandbox can't push tags, awaiting maintainer sweep. (c) MAY-4.md Concern 3 (navigable streaming response queue) — naturally subsumed by SessionModel + ReplPathway (Phase 2). (d) Manual NVDA matrix walkthrough of the Cycle 28 Window-menu rows + Cycle 29b SelectionProfile rows in `docs/ACCESSIBILITY-TESTING.md` against the next preview cut. **Cycle 27 NVDA matrix is validated** as of 2026-05-09 ("multi-state menu items work exactly as expected"). |
