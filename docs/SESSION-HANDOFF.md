...
| **In-flight branch** | `claude/resume-and-plan-gMPNJ` — **Cycle 27 multi-state menu paradigm** (this PR). Single-PR architectural cycle establishing a canonical paradigm for stateful menu operations + migrating the two existing single-action toggles whose semantics fit the paradigm (`MuteEarcons` → `EarconsMode`, `ToggleDebugLog` → `LoggingLevel`). Drops the two keyboard accelerators (`Ctrl+Shift+M`, `Ctrl+Shift+G`) per the maintainer's hotkey-count working-memory ceiling — both operations are now menu-only. Adds `MultiStateCommand` DU + `bindMultiState` helper + `SubmenuOpened`-driven `IsChecked` refresh for active-option indication readable to NVDA as UIA TogglePattern. New `MultiStateRegistryTests.fs` fixtures + `AppReservedHotkeysMirrorTests` parity preserved automatically. Most recent merge before this branch: PR #228 (Cycle 26d, commit `163bd28`). |
| **Next stage** | **Decide next cycle direction.** Cycle 27 closes; multi-state menu paradigm is shipped, two existing toggles migrated, hotkey count down by two. Two strong candidates on deck per [`docs/PROJECT-PLAN-2026-05-09.md`](PROJECT-PLAN-2026-05-09.md): **(a) Output framework cycle (Part 3)** — subsumes `spec/tech-plan.md` Stages 8 + 9; designed in `spec/event-and-output-framework.md`; multi-cycle work; plan-mode per sub-stage. The strategic priority. **(b) Future menus and per-output settings** — the maintainer's anticipated additions (Window menu, Display, Preferences/Settings with first user-toggle "hide CWD on cmd") would exercise both the Cycle 26 extension recipe AND the Cycle 27 multi-state paradigm end-to-end before the framework cycle's heavier cadence. A natural near-term application of the Cycle 27 paradigm is migrating Shell-switch (`Ctrl+Shift+1/2/3`) into a multi-state command with per-option keyboard accelerators (would require extending `MultiStateOption` with optional `Key`/`Modifiers` — out of scope for Cycle 27 itself but flagged as the obvious next consumer). **Parked**: Cycle 25b-2 (FlaUI/UIA E2E runner). **Open follow-ups (cross-cutting; not blocking the next cycle)**: (a) Screen-buffer runtime resize (Phase 2). (b) Stable-baseline tag pushes including `baseline/stage-7-claude-roundtrip`, the Cycle-24e candidate, `baseline/cycle-26-app-menu`, and the new `baseline/cycle-27-multistate-paradigm` per [`docs/CHECKPOINTS.md`](CHECKPOINTS.md) — local sandbox can't push tags, awaiting maintainer sweep. (c) MAY-4.md Concern 3 (navigable streaming response queue) — naturally subsumed by SessionModel + ReplPathway (Phase 2). (d) Manual NVDA matrix walkthrough of the new Cycle 27 rows in `docs/ACCESSIBILITY-TESTING.md` against the next preview cut. |
