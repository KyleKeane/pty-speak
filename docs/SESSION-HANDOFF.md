...
| **In-flight branch** | `claude/cycle-28-window-menu-and-cleanup` — **Cycle 28 Window menu + foundation cleanup** (this PR). Mixed cleanup + small-feature cycle before the framework cycle ramps. Adds the `Window` top-level menu (`Close Window` + `Exit` as new menu-only AppCommands `CloseWindow` / `ExitApp`); fixes a stale `Ctrl+Shift+L` comment in `Program.fs:685` (replaced with live `Ctrl+Shift+P` reference); refreshes `docs/PROJECT-PLAN-2026-05-09.md` in place with "Status update 2026-05-09 (post Cycle 27 closure)" callout + Sequencing block updates marking Cycles 26-28 as ✅ shipped; carries an informational hotkey-menu-coverage audit confirming all 12 remaining gesture-bearing AppCommands ARE represented in the menu (none are orphaned). Most recent merge before this branch: PR #229 (Cycle 27, commit `1262dbb`). |
| **Next stage** | **Output framework cycle (Part 3)** per [`docs/PROJECT-PLAN-2026-05-09.md`](PROJECT-PLAN-2026-05-09.md) Sequencing — subsumes `spec/tech-plan.md` Stages 8 + 9; designed in `spec/event-and-output-framework.md`; multi-cycle work; plan-mode cycle per sub-stage. **First sub-stage: a plan-mode RFC** consulting [`docs/STAGE-7-ISSUES.md`](STAGE-7-ISSUES.md) + [`spec/event-and-output-framework.md`](../spec/event-and-output-framework.md) with framework-design intent — the act of reading these docs WITH design intent is more valuable than reading them as cleanup, so the framework cycle should drive the alignment pass rather than a pre-cycle audit. Per-output-class user toggles (e.g. earcon-volume, suppression-rules) can land in the menu surface established by Cycles 26 + 27 without burning hotkey slots. **Other near-term candidates** (smaller / parked): (a) Migrating Shell-switch (`Ctrl+Shift+1/2/3`) into a multi-state command with per-option keyboard accelerators — natural near-term consumer of the Cycle 27 paradigm; would require extending `MultiStateOption` with optional `Key`/`Modifiers`. (b) Preferences/Settings menu (first user-toggle: hide CWD on cmd). **Parked**: Cycle 25b-2 (FlaUI/UIA E2E runner). **Open follow-ups (cross-cutting; not blocking)**: (a) Screen-buffer runtime resize (Phase 2). (b) Stable-baseline tag pushes including `baseline/stage-7-claude-roundtrip`, the Cycle-24e candidate, `baseline/cycle-26-app-menu`, `baseline/cycle-27-multistate-paradigm`, and the new `baseline/cycle-28-window-menu-and-cleanup` per [`docs/CHECKPOINTS.md`](CHECKPOINTS.md) — local sandbox can't push tags, awaiting maintainer sweep. (c) MAY-4.md Concern 3 (navigable streaming response queue) — naturally subsumed by SessionModel + ReplPathway (Phase 2). (d) Manual NVDA matrix walkthrough of the new Cycle 28 Window-menu rows in `docs/ACCESSIBILITY-TESTING.md` against the next preview cut. **Cycle 27 NVDA matrix is validated** as of 2026-05-09 ("multi-state menu items work exactly as expected"). |
