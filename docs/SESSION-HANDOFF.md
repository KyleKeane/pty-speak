...
| | |
|---|---|
| **Last merged stages** | **Stages 0 → 7 + 11** all merged to `main`, plus the retroactively-formalized Stages 4a / 4b / 5a. Stage 7 shipped 2026-05-03 across 11 sequenced PRs (A → K, #131-#143) + PR-L #144 (doc-purpose audit + Stage 7 wrap-up) + PR-M #145 (coalescer cross-row spinner gate fix). **Post-Stage-7 substrate cycle 2026-05-04 → 2026-05-07** (PRs #146-#184) shipped: event-and-output framework spec (#151); Stages 8a/8b/8c/8d.1/8d.2 (#152-#157, #155); display-pathway substrate Phase A + Phase A.1 + Phase B subset + Phase A.2 + #166 suffix-diff + #167 atlas + #168 Tier 1 parameters + #169 shell-switch flush fix (#159-#169); five research-stage docs: PIPELINE-NARRATIVE (#170), SESSION-MODEL (#171), INTERACTION-MODEL (#173), PANE-MODEL (#174), CUSTOMIZATION-MODEL (#181); six-track audit phase (#172, #175-#180); post-audit cleanup (#181-#184). **Tier 1 SessionModel implementation cycle 2026-05-07 → 2026-05-08** (PRs #185-#199) shipped substrate end-to-end: skeleton (#185), OSC 133 producer + cursor field (#186), state machine + composition (#187), heuristic fallback + alt-screen wiring (#189), PromptText capture (#190), diagnostic battery extension (#191), tick-driven detector via channel-driven actor model (#192), CHANNEL-ARCHITECTURE doc (#193), default-shell config + detector consolidation (#194), row-index-aware emission (#195) + announce-wording followup (#196), CommandText + OutputText extraction (#197), multi-match flap fix (#198), Ctrl+Shift+Y clipboard hotkey (#199). Maintainer NVDA-validation green throughout. **Cycle 24 SessionModel persistence cycle 2026-05-09** (PRs #203, #206-#212, #219) shipped Tier 2 end-to-end: 24a TOML schema (#203), 24b JSONL serializer (#206), 24c bounded-channel async file writer (#208), 24d-1 Always-mode synchronous flush (#209), 24d-2 env-var value sanitisation (#210), 24e Ctrl+Shift+S diagnostic hotkey + NVDA matrix (#211), 24f persistence-config diagnostic + matrix typo fix (#212), 24g TOML model snapshot logged at startup (#219). **Cycle 25 operational-ergonomics + diagnostic-dump bundle 2026-05-09** (PRs #220-#222): 25a hotkey reorg (Ctrl+Shift+L/P/E rebound) + open-config auto-create + reload-on-shell-switch + `[logging]` TOML (#220); 25b-1 Ctrl+Shift+D bundles diagnostic dump (FileLogger log + config + redacted env into dated snapshot file + clipboard) + Ctrl+Shift+T placeholder removed (#221); 25b-1a bundle gains `--- SESSION LOG ---` section + Ctrl+Shift+L removed since D's bundle subsumes it (#222). **Cycle 25c doc-only handoff refresh 2026-05-09** (PR #223): refreshes "Where we left off" through Cycle 25b-1a; "Next stage" cell repointed at the candidate-cycle picker. **Cycle 25d dated-snapshot plan refresh 2026-05-09** (PR #224): spawns `docs/PROJECT-PLAN-2026-05-09.md` per E5 discipline; sweeps `-revision.md` references in 5 active files; closes the overdue dated-revision follow-up. **Cycle 26 app menu cycle 2026-05-09** (PRs #225-#228): 26a menu skeleton + UIA plumbing (#225); 26b option-types `Hotkey.Key`/`Modifiers`, makes `bindHotkey` return `RoutedCommand`, populates the 5-menu structure (Shell/View/Data/Diagnostics/Help) with 14 named MenuItems wired to the same RoutedCommand instances as the keyboard hotkeys via reflection-driven compose-time wiring, adds `AppReservedHotkeysMirrorTests.fs` to pin the F#/C# parity invariant at test time (#226); 26c first menu-only AppCommand `RunProcessCleanupScript` surfaced via Diagnostics → Test Process Cleanup (no keyboard accelerator) — proves the option-typed plumbing works end-to-end (#227); 26d NVDA matrix rows + "How to add a menu item" extension recipe in `USER-SETTINGS.md` (#228). **Cycle 27 multi-state menu paradigm 2026-05-09** (PR #229): introduces parallel `MultiStateCommand` DU + `MultiStateDef`/`MultiStateOption` records + `multiStateBuiltIns`/`multiStateAllCommands`/`multiStateOf`/`multiStateNameOf` registry surfaces in `HotkeyRegistry.fs`; new `bindMultiState` helper + `MultiStateBinding` capture in `Program.fs` mirrors `bindHotkey`'s shape minus the `KeyBinding` step (multi-state is menu-only by canon as of Cycle 27); migrates `MuteEarcons` (Ctrl+Shift+M) → `EarconsMode` (View → Earcons → Enabled / Muted) and `ToggleDebugLog` (Ctrl+Shift+G) → `LoggingLevel` (View → Logging Level → Information / Debug); both keyboard accelerators dropped per the maintainer's hotkey-count working-memory ceiling; new `MultiStateRegistryTests.fs` with 11 fixtures pins the same shape contracts that `HotkeyRegistryTests` pins for `AppCommand`; `AppReservedHotkeysMirrorTests` parity preserved automatically (both surfaces shed the same two entries); active option indicated via WPF `MenuItem.IsCheckable=true` + `IsChecked` (refreshed on `SubmenuOpened` from each binding's `GetCurrent` closure), surfacing UIA TogglePattern that NVDA reads as "menu item, checked" / "menu item, not checked"; **maintainer NVDA-validated 2026-05-09** ("multi-state menu items work exactly as expected") against the post-Cycle-27 release build. **Cycle 28 Window menu + foundation cleanup 2026-05-09** (PR #230, commit `5329fa3`): adds top-level `Window` menu with `Close Window` (calls `Window.Close()`; visual `InputGestureText="Alt+F4"` for the OS-handled gesture) and `Exit` (`Application.Current.Shutdown()`); both surfaces as new menu-only AppCommands (`CloseWindow`, `ExitApp`); fixes a stale `Ctrl+Shift+L` reference in `Program.fs:685` (replaced with the live `Ctrl+Shift+P` → `OpenDataFolder`; the old hotkey was removed in Cycle 25b-1a); refreshes `docs/PROJECT-PLAN-2026-05-09.md` in place with a "Status update 2026-05-09 (post Cycle 27 closure)" callout + Sequencing block updated to mark Cycles 26-28 as ✅ shipped; informational hotkey-menu-coverage audit confirmed all 12 remaining gesture-bearing AppCommands ARE represented in the menu (none are menu-orphaned). **Cycle 29a SelectionDetector substrate 2026-05-09** (this PR): first of three sequenced PRs (29a/29b/29c) closing spec Stage 8e for `[output-selection]` per `docs/STAGE-7-ISSUES.md:337`. Ships pure functional `SelectionDetector.fs` implementing tech-plan §8.1 detection heuristic (two-pass: PASS 1 row-classification + region-grouping; PASS 2 signal aggregation: stable region after `HighlightDetectionThresholdMs`, SGR-distinct by construction, arrow-key correlation upgrades confidence to `HeuristicSGRWithKeystroke`). Public API: `Direction` / `SelectionSource` / `Parameters` / `defaultParameters` / `create` / `tryDetect` / `feedKeystroke` / `reset`. Mirrors `HeuristicPromptDetector.fs` shape; per-shell short-circuit (claude-only). New `SelectionExtensions` constants module in `OutputEventTypes.fs` declares well-known `OutputEvent.Extensions` keys (`selection.itemCount`, `selection.selectedIndex`, `selection.itemText`, `selection.allItems`, `selection.topRow`, `selection.bottomRow`, `selection.source`) — schema-stable across the sub-cycle; 8e-B's UIA peer queries the same keys. New `SelectionDetectorTests.fs` with 19 facts pinning the contract: per-shell activation gate, stability window, initial-burst shape (Producer + Priority + Extensions), selection-index update via signature gate, suppression on identical snapshot, dismissal grace, confidence-tier upgrade on arrow-key correlation, `MinConfidence` gating, region-rejection (single-row / 8-row / SGR-uniform), reset behaviour. **The detector is NOT yet wired into `OutputDispatcher` — that's Cycle 29b. NVDA behaviour on the running app is unchanged by Cycle 29a.** Substrate-only-first decomposition lets the heuristic be code-reviewed and CI-tested in isolation before binding to NVDA. **Cycle 29b SelectionProfile + Program.fs wiring 2026-05-09** (this PR): NVDA starts speaking selection prompts as text. New `SelectionProfile.fs` (cousin of `EarconProfile.fs`) pattern-matches on `SelectionShown` / `SelectionItem` / `SelectionDismissed` and emits NVDA + FileLogger `RenderText` ChannelDecisions. **Empty-payload trick** (mirrors 8d.2 ErrorLine/WarningLine): the detector now emits with empty `Payload` so `PassThroughProfile`'s catch-all NVDA decision is skipped (NvdaChannel.fs:87 drops `RenderText ""`); SelectionProfile then constructs the user-facing text from structured `Extensions` data and emits the ONE NVDA + FileLogger pair the user actually hears. Detector adjustments: empty Payload + new `ItemIndex` extension key on burst SelectionItem events (was conflated with `SelectedIndex` in 29a — burst items now carry `SelectedIndex` constant + `ItemIndex` per-item). Profile rendering: `SelectionShown` formats "selection prompt: Edit, Yes, Always, No (selected: Yes)"; `SelectionItem` formats "selected: Yes, 2 of 4" (when `ItemIndex == SelectedIndex`) or "Edit, 1 of 4" (when not); `SelectionDismissed` renders literal "selection dismissed". `Program.fs` composition wiring: `selectionProfile` registers alongside passthrough + earcon; `let mutable selectionDetector` adjacent to `promptDetector`; `runDetector` extension calls `SelectionDetector.tryDetect` after the existing `HeuristicPromptDetector.tryDetect` and routes each emitted event directly to `OutputDispatcher.dispatch`; reset hooks at all 3 sites that already reset `promptDetector` (alt-screen entry/exit, shell-switch). New `SelectionProfileTests.fs` with 13 facts pins the profile contract. Updated `SelectionDetectorTests.fs` with `ItemIndex` schema fact + payload-empty assertions. **Keystroke wiring deferred to 8e-C** per maintainer's plan-mode choice — the detector emits at default `HeuristicSGR` confidence without keystroke input, sufficient for end-to-end NVDA payoff. |
...
| **In-flight branch** | `claude/cycle-29b-test-followup` — **Cycle 29b NVDA-test follow-up** (this PR; doc-only). Captures three lessons from the 2026-05-09 NVDA validation pass on the post-Cycle-29b release build: (1) `CLAUDE.md` now warns future Claude sessions to request log chunks (specific Semantic categories, time-bounded windows, named bundle sections) rather than full diagnostic bundles — the Cycle 29b test produced a bundle large enough to crash the maintainer's iOS chat app on paste. (2) `docs/STAGE-7-ISSUES.md` `[output-stream]` and `[output-earcon]` entries get **empirical confirmation** notes from the test: Claude's thinking-state spinner animations (`✻ Transmuting…` / rotating glyph + incrementing token counter) defeat the existing identical-hash spinner gate, producing ~80 announces per Claude turn; the same red-coloured spinner glyphs trigger ~30 `error-tone` earcons per turn via the color-detection ErrorLine heuristic firing as false positive. Both gaps tracked as near-term candidates with concrete fix sketches (`StreamPathway` Levenshtein-based spinner gate + spinner-glyph-aware color-detection refinement; ~80-150 LOC each). (3) `[output-selection]` entry gets a **validation-gotcha** note: Claude's auto-trust mode for known directories skips the tool-use confirmation prompt entirely, so 29b's SelectionProfile didn't get exercised in the test — the cmd / PowerShell short-circuit DID confirm cleanly (zero `SelectionShown` events across a 5-minute non-Claude session). Future testers need to ask Claude to write/edit a file (not just read), or run pty-speak from a directory Claude hasn't been granted trust in, to force the prompt to render. Most recent merge before this branch: PR #232 (Cycle 29b, commit `81ba608`). |
| **Next stage** | **Decide between two near-term candidates.** **Path A (Cycle 29c TOML loader)** finishes the Stage 8e-A scope as planned: `[profile.selection]` section in `Config.fs` so the maintainer can tune `HighlightDetectionThresholdMs` / `DismissalGraceMs` / `KeystrokeCorrelationWindowMs` / `MinConfidence` via `Ctrl+Shift+E`; ~250 LOC; clean substrate completion but no immediate user-visible payoff. **Path B (spinner storm + red-tone fixes)** addresses the two `[output-stream]` and `[output-earcon]` STAGE-7-ISSUES gaps that the 2026-05-09 NVDA test empirically confirmed are the load-bearing UX blockers for Claude sessions. Both fixes are isolated to `StreamPathway` (~50-100 LOC for spinner gate + ~30-50 LOC for color-detection refinement); neither needs the full framework cycle. Recommendation per maintainer's 2026-05-09 chat: **Path B first** — UIA listbox semantics in 8e-B can't be properly tested while spinner-storm noise drowns out the prompt. After Path B, return to 29c then 8e-B. **Cycle 8e-B + 8e-C** continue the sub-cycle: UIA listbox peer (`TerminalListAutomationPeer` + `TerminalListItemAutomationPeer`; replaces text-only RenderText with RenderRaw UIA listbox semantics; spec tech-plan §8.2-§8.5) + arrow-key round-trip (`ISelectionItemProvider.Select` → PTY arrow byte sequence + the deferred keystroke-feedback wiring). Each gets its own plan-mode pass. **Open follow-ups (cross-cutting; not blocking)**: (a) Screen-buffer runtime resize (Phase 2). (b) Stable-baseline tag pushes including `baseline/stage-7-claude-roundtrip`, the Cycle-24e candidate, `baseline/cycle-26-app-menu`, `baseline/cycle-27-multistate-paradigm`, `baseline/cycle-28-window-menu-and-cleanup`, `baseline/cycle-29a-selectiondetector-substrate`, and `baseline/cycle-29b-selectionprofile-wiring` per [`docs/CHECKPOINTS.md`](CHECKPOINTS.md) — local sandbox can't push tags, awaiting maintainer sweep. (c) MAY-4.md Concern 3 (navigable streaming response queue) — naturally subsumed by SessionModel + ReplPathway (Phase 2); 2026-05-09 NVDA test surfaced its practical impact ("trickles results down" was specifically called out by maintainer). (d) **Better mechanism for sharing huge log files** — the Cycle 29b NVDA test's bundle crashed the iOS chat app on paste; CLAUDE.md now codifies chunk-first triage but a longer-term fix (gist upload? shareable pastebin? bundle-splitter hotkey?) is worth designing when bandwidth allows. **NVDA validation status**: Cycle 27 multi-state menu items validated 2026-05-09 ("work exactly as expected"). Cycle 28 Window menu validated 2026-05-09 ("everything worked as expected, including test 1-5"). Cycle 29b SelectionProfile **code-clean but not exercised** — selection prompt didn't render due to Claude auto-trust; cmd / PowerShell short-circuit confirmed working. |
