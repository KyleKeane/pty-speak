...
| `Engine.Core` | RELAUNCH-SPEC Phase 0 — the interaction engine's pure core (ADR 0011 E1; plain `net9.0`, platform-free, CI-enforced) | `Chunk`/`ChunkTree` (the §5 chunk-tree data model: durable ids, capture + authored order, anchor-nested branches); `AgentEvent` + `ClaudeStreamJson` (typed participant-stream vocabulary + tolerant stream-json parser); `MarkdownChunker` (Markdig block-level decomposition — the chunk grain); `EngineEvent`/`EngineBus` (the universal event bus, instance-scoped); `Ingest` (the §5.2 streaming rule as a pure fold); `Navigator` (§6.2 verbs, §6.4 non-ejection by construction); `ChunkNarration` (canonical self-voicing rendering); `Attention` (§7.3 foreground/ambient queue + routing policy); `ISpeechSink`; `SemanticOutline` (ADR 0012 S1 heading-scoped outline); `SpatialCue` (ADR 0012 S3 stage signatures); `EngineConfig` (engine.toml, warn-and-default — ADR 0014 C1); `EngineDiagnostics` (the ear-first ring — C4); `Notebook` (the authored layer — ADR 0013 N1/N3); `KeyMap` (the declarative verb table — C2); `ChunkSerde` + `NotebookSerde` (schema-v1 JSONL + validated `ChunkTree.restore` — N4) | ✅ shipped (Phase 0 #444–#451 + ADR 0012 cycle, 2026-06-11/12; dogfood pending) |
| `Engine.Participants` | Participant-seam transport (spec §0.2/§12) | `ClaudeCli` — pure `buildArguments` (per-turn `-p … stream-json --verbose` + `--resume`) + pure `pumpLines` + thin `runTurn` process layer (concurrent stderr drain, typed `TurnOutcome`) | ✅ shipped (#450) |
| `Engine.Voice` | Self-voicing channel back-end (spec §4.6) | `SapiSink` — Windows SAPI (`System.Speech`) behind `ISpeechSink`; completion fires on finish AND cancel | ✅ shipped (#451) |
| `Engine.Audio` | Spatial-cue renderer (ADR 0012 S4) | `SpatialPlayer` — renders `Engine.Core.SpatialCue.Cue` as stereo-panned sine tones (NAudio `PanningSampleProvider`; per-play `WasapiOut` per the Terminal.Audio lesson); a universal-event-bus consumer, never gating speech | ✅ shipped (Cycle 53 ADR 0012) |
| `Engine.Host` | Phase 0 console host (ADR 0011 E8) | Key loop (compose / branch / anchor-return / jump / tree nav / repeat / stop) → participant turns on a background thread → ingest → bus → attention drain → sink. `ENGINE_CLAUDE_PATH` env override | ✅ shipped (#451; validation = the local dogfood, `P0-ENGINE-1`) |
